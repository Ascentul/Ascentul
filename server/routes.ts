...
- Projects relevant to the company's industry or technology stack`
  ...
  apiRouter.get("/admin/metrics/overview", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const [
        ...
        storage.getUnreadContactMessagesCount()
      ]);
  ...
  apiRouter.get("/admin/users/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const [
        ...
        storage.getResumes(userId),
        storage.getCoverLetters(userId),
        storage.getInterviewProcesses(userId),
        storage.getXpHistory(userId)
      ]);
      ...
      res.status(200).json({
        ...
        stats: {
          ...
          interviewProcessCount: interviewProcesses.length,
          xpTotal: user.xp || 0,
          xpHistory: xpHistory.slice(0, 10) // Latest 10 XP entries
        }
      });
    } catch (error) {
  ...
  apiRouter.put("/admin/users/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
      const { name, email, userType, subscriptionPlan, subscriptionStatus } = req.body;
  ...
  apiRouter.get("/admin/analytics/user-growth", requireAdmin, async (req: Request, res: Response) => {
    try {
      const users = await storage.getAllActiveUsers();
      
      // Group users by signup date (month/year)
      ...
      // Convert to array of objects for chart data
      const chartData = Object.entries(usersByMonth).map(([monthYear, count]) => {
        const [year, month] = monthYear.split('-');
        return {
          month: `${new Date(parseInt(year), parseInt(month) - 1).toLocaleString('default', { month: 'short' })} ${year}`,
          users: count
        };
      }).sort((a, b) => {
        // Sort by month/year
      ...
      res.status(500).json({ message: "Error fetching user growth analytics" });
    }
  });
  
  apiRouter.get("/admin/analytics/subscription-trends", requireAdmin, async (req: Request, res: Response) => {
    try {
      const users = await storage.getAllActiveUsers();
      ...
      const statusChartData = Object.entries(subscriptionsByStatus).map(([status, count]) => ({
      ...
      res.status(500).json({ message: "Error fetching subscription trends" });
    }
  });
  
  apiRouter.get("/admin/analytics/feature-usage", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const [
        ...
        allResumes,
        allCoverLetters,
        allInterviewProcesses,
        allXpHistory
      ] = await Promise.all([
        storage.getAllActiveUsers(),
        storage.getAllGoals(),
        storage.getAllWorkHistory(),
        storage.getAllResumes(),
        storage.getAllCoverLetters(),
        storage.getAllInterviewProcesses(),
        storage.getAllXpHistory()
      ]);
      
      // Calculate feature usage metrics
      const totalUsers = users.length;
      const usersWithGoals = new Set(allGoals.map(goal => goal.userId)).size;
      const usersWithResumes = new Set(allResumes.map(resume => resume.userId)).size;
      const usersWithCoverLetters = new Set(allCoverLetters.map(letter => letter.userId)).size;
      const usersWithInterviews = new Set(allInterviewProcesses.map(process => process.userId)).size;
      const usersWithWorkHistory = new Set(allWorkHistory.map(work => work.userId)).size;
      
      // Calculate average items per user for each feature
      const avgGoalsPerUser = totalUsers ? allGoals.length / totalUsers : 0;
      const avgResumesPerUser = totalUsers ? allResumes.length / totalUsers : 0;
      const avgCoverLettersPerUser = totalUsers ? allCoverLetters.length / totalUsers : 0;
      const avgInterviewsPerUser = totalUsers ? allInterviewProcesses.length / totalUsers : 0;
      const avgWorkHistoryPerUser = totalUsers ? allWorkHistory.length / totalUsers : 0;
      
      // Calculate adoption rates (percentage of users using each feature)
      const goalAdoptionRate = totalUsers ? (usersWithGoals / totalUsers) * 100 : 0;
      const resumeAdoptionRate = totalUsers ? (usersWithResumes / totalUsers) * 100 : 0;
      const coverLetterAdoptionRate = totalUsers ? (usersWithCoverLetters / totalUsers) * 100 : 0;
      const interviewAdoptionRate = totalUsers ? (usersWithInterviews / totalUsers) * 100 : 0;
      const workHistoryAdoptionRate = totalUsers ? (usersWithWorkHistory / totalUsers) * 100 : 0;
      
      // Format data for charts
      const adoptionRateData = [
        { name: 'Goals', value: goalAdoptionRate },
        { name: 'Resumes', value: resumeAdoptionRate },
        { name: 'Cover Letters', value: coverLetterAdoptionRate },
        { name: 'Interview Tracking', value: interviewAdoptionRate },
        { name: 'Work History', value: workHistoryAdoptionRate }
      ];
      
      const itemsPerUserData = [
        { name: 'Goals', value: avgGoalsPerUser },
        { name: 'Resumes', value: avgResumesPerUser },
        { name: 'Cover Letters', value: avgCoverLettersPerUser },
        { name: 'Interview Tracking', value: avgInterviewsPerUser },
        { name: 'Work History', value: avgWorkHistoryPerUser }
      ];
      
      // Calculate XP source distribution
      const xpBySource: Record<string, number> = {};
      allXpHistory.forEach(entry => {
        if (!xpBySource[entry.source]) {
          xpBySource[entry.source] = 0;
        }
        xpBySource[entry.source] += entry.amount;
      });
      
      const xpSourceData = Object.entries(xpBySource).map(([source, amount]) => ({
        name: source.replace(/_/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
        value: amount
      })).sort((a, b) => b.value - a.value);
      
      res.status(200).json({
        adoptionRates: adoptionRateData,
        itemsPerUser: itemsPerUserData,
        xpDistribution: xpSourceData
      });
    } catch (error) {
      console.error("Error fetching feature usage analytics:", error);
      res.status(500).json({ message: "Error fetching feature usage analytics" });
    }
  });
  ...
  // Admin Dashboard API routes for user metrics
  apiRouter.get("/users/total", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  apiRouter.get("/users/new", requireAdmin, async (req: Request, res: Response) => {
    try {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
  ...
  apiRouter.get("/users/premium", requireAdmin, async (req: Request, res: Response) => {
    try {
      const users = await storage.getAllUsers();
  ...
  apiRouter.get("/users/active", requireAdmin, async (req: Request, res: Response) => {
    try {
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      
      // For now, use a simple active user calculation
      const users = await storage.getAllUsers();
      // In a real implementation, we'd check login history or user activity logs
      const activeUsers = users.length > 0 ? Math.ceil(users.length * 0.7) : 0;
      
      res.json({ total: activeUsers });
    } catch (error) {
  ...
  apiRouter.get("/system/status", requireAdmin, async (req: Request, res: Response) => {
    try {
      // Calculate system uptime (in days)
      const startTime = process.uptime();
      const uptimeDays = Math.floor(startTime / (60 * 60 * 24));
      ...
      const dbStatus = dbConnected ? "healthy" : "down";
      ...
      res.json({
        ...
        services: {
          database: { 
            status: dbStatus,
            details: dbConnected ? "Connected" : "Connection failed"
          },
          api: { status: "healthy" },
          auth: { status: "healthy" }
        }
      ...
      res.status(500).json({
        ...
        version: "unknown",
        services: {
          database: { status: "down", details: error.message },
          api: { status: "degraded" },
          auth: { status: "unknown" }
        }
      });
  ...
  // Support messages API endpoint
  apiRouter.get("/support/unread", requireAdmin, async (req: Request, res: Response) => {
    try {
      const messages = await storage.getContactMessages();
      const unreadMessages = messages.filter(msg => !msg.read && !msg.archived);
      
      res.json({ 
        total: unreadMessages.length,
        messages: unreadMessages.slice(0, 5) // Return only the first 5 for preview
      });
    } catch (error) {
      console.error("Error getting unread support messages:", error);
      res.status(500).json({ error: "Failed to get unread support messages" });
    }
  });

  // Database connection check endpoint
  apiRouter.get("/health/database", requireAdmin, async (req: Request, res: Response) => {
    try {
      const isConnected = await storage.checkDatabaseConnection();
