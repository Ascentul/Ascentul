...
export const setupAdvisorTestData = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (!advisorUser) {
      console.log(
        '⚠️  Advisor user not found. Please sign in with test.advisor@ascentful.io first to create profile.',
      );
    ...
    if (students.length === 0) {
      console.log(
        '⚠️  No student users found. Please sign in with test.student1@ascentful.io and test.student2@ascentful.io first.',
      );
    ...
    for (const student of students) {
      ...
      if (!existingProfile) {
        await ctx.db.insert('studentProfiles', {
          ...
          university_id: university._id,
          major:
            student.email === 'test.student1@ascentful.io'
              ? 'Computer Science'
