  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (afterRealMetrics.totalUniversitiesAllTime === expectedTotal) {
        ...
        log(
          `  ❌ FAIL: Expected ${expectedTotal}, got ${afterRealMetrics.totalUniversitiesAllTime}`,
        );
      }
      ...
      if (afterRealMetrics.activeUniversitiesCurrent === expectedActive) {
        ...
        log(
          `  ❌ FAIL: Expected ${expectedActive}, got ${afterRealMetrics.activeUniversitiesCurrent}`,
        );
      }
      ...
      if (
        afterArchiveMetrics.activeUniversitiesCurrent === initialMetrics.activeUniversitiesCurrent
      ) {
        log(`  ✓ PASS: Active universities decreased (archived removed from active count)`);
      } else {
        log(
          `  ❌ FAIL: Expected ${initialMetrics.activeUniversitiesCurrent}, got ${afterArchiveMetrics.activeUniversitiesCurrent}`,
        );
      }
      ...
      if (afterArchiveMetrics.archivedUniversities === expectedArchived) {
        ...
        log(
          `  ❌ FAIL: Expected ${expectedArchived}, got ${afterArchiveMetrics.archivedUniversities}`,
        );
      }
      ...
      log(
        `   1. Toggle to test: npx convex run universities:toggleTestUniversity --universityId "${realUniversityId}" --isTest true`,
      );
      log(
        `   2. Hard delete: npx convex run universities:hardDeleteUniversity --universityId "${realUniversityId}"`,
      );
