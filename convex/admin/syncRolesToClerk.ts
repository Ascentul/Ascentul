...
export const syncAllRolesToClerk = action({
  ...
  handler: async (ctx, args) => {
    ...
    const callerResponse = await fetch(`https://api.clerk.com/v1/users/${args.clerkId}`, {
      headers: {
    ...
    console.log(
      `[SyncRolesToClerk] Authorized: ${maskEmail(caller.email_addresses?.[0]?.email_address) || args.clerkId} (super_admin)`,
    );
    ...
    do {
      const page: {
        users: Array<{ _id: string; clerkId: string; email: string; name: string; role: string }>;
        cursor: string | null;
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        if (!isValidUserRole(user.role)) {
          ...
          console.error(
            `[SyncRolesToClerk] Invalid role for ${maskEmail(user.email)}: ${user.role}`,
          );
          continue;
        }
        ...
        if (!user.clerkId || user.clerkId === '' || user.clerkId.startsWith('pending_')) {
          ...
          results.details.push({
            ...
            clerkRole: null,
            action: 'skipped',
            message: user.clerkId?.startsWith('pending_') ? 'Pending activation' : 'No Clerk ID',
          });
          continue;
        }
        ...
        const freshResponse = await fetch(`https://api.clerk.com/v1/users/${user.clerkId}`, {
          headers: {
            Authorization: `Bearer ${CLERK_SECRET_KEY}`,
            'Content-Type': 'application/json',
          },
        ...
        const updateResponse = await fetch(`https://api.clerk.com/v1/users/${user.clerkId}`, {
          method: 'PATCH',
          headers: {
            Authorization: `Bearer ${CLERK_SECRET_KEY}`,
            'Content-Type': 'application/json',
        ...
        console.log(
          `[SyncRolesToClerk] Synced ${maskEmail(user.email)}: ${clerkRole} → ${user.role}`,
        );
      } catch (error) {
        results.errors++;
        results.details.push({
      ...
      if ((i + 1) % BATCH_SIZE === 0 && i < users.length - 1) {
        ...
        console.log(
          `[SyncRolesToClerk] Progress: ${processed}/${users.length} users processed (${results.synced} synced, ${results.skipped} skipped, ${results.errors} errors). ${remaining} remaining. Pausing for ${DELAY_MS}ms...`,
        );
