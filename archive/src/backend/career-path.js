...
function mockCareerPathResponse(jobTitle) {
    return {
        paths: [
            {
                id: "main-path",
...
export function registerCareerPathRoutes(app) {
    // Generate a career path from job title
    app.post("/api/career-paths/generate", async (req, res) => {
        try {
    ...
    app.delete("/api/career-paths/:id", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
