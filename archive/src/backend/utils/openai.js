...
export async function generateCoachingResponse(messages, userContext) {
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                ...
                {
                    role: "user",
                    content: "Generate today's personalized career recommendations based on my profile"
                }
            ],
            temperature: 0.7,
            max_tokens: 500,
...
export async function generateAIResponse(prompt, selectedModel) {
    try {
        // Validate the selected model or use default
...
export async function cleanOptimizedCoverLetter(optimizedLetter) {
