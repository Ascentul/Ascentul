...
if (!stripeKey) {
    ...
    stripe = new Stripe(stripeKey, {
        apiVersion: '2023-10-16', // Casting as any to bypass type checking
    });
}
...
function getSubscriptionExpiryDate(interval) {
    const now = new Date();
    switch (interval) {
        case 'monthly':
...
export async function createSubscription(data) {
    try {
        ...
        const customerId = await createOrRetrieveCustomer(data.userId, data.email, data.userName);
        // Check if this is a mock customer ID (for demo purposes)
...
export async function handleSubscriptionUpdated(subscriptionId) {
    try {
...
export async function createSetupIntent(userId) {
    try {
        ...
        const customerId = await createOrRetrieveCustomer(userId, user.email, user.name);
        // Create setup intent
