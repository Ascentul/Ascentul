...
export const registerContactsRoutes = (app, storage) => {
    ...
    app.get("/api/contacts/need-followup", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            if (!userId) {
                return res.status(401).json({ message: "Unauthorized" });
    ...
    app.get("/api/contacts/all-followups", requireLoginFallback, async (req, res) => {
        try {
    ...
    app.get("/api/contacts/:id", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
    ...
    app.post("/api/contacts", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            if (!userId) {
                return res.status(401).json({ message: "Unauthorized" });
    ...
    app.put("/api/contacts/:id", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
            if (!userId) {
    ...
    // Delete a contact
    app.delete("/api/contacts/:id", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
    ...
    app.post("/api/contacts/:id/log-interaction", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
            if (!userId) {
            ...
            const newInteraction = await storage.createContactInteraction(userId, contactId, {
                contactId: contactId,
    ...
    app.get("/api/contacts/:id/interactions", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
            if (!userId) {
    ...
    app.put("/api/contacts/interactions/:id", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
    ...
    app.delete("/api/contacts/interactions/:id", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const interactionId = parseInt(req.params.id);
            if (!userId) {
    ...
    app.post("/api/contacts/:id/schedule-followup", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
            if (!userId) {
            ...
            if (req.body.followUpDate) {
                if (req.body.followUpDate instanceof Date) {
                    ...
                    return res.status(400).json({
                        message: "Follow-up date must be a valid date string or Date object"
                    });
                }
            }
    ...
    app.get("/api/contacts/:id/followups", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
            const contactId = parseInt(req.params.id);
            if (!userId) {
    ...
    app.put("/api/contacts/followups/:followupId/complete", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
    ...
    app.delete("/api/contacts/:contactId/followups/:followupId", requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId || req.user?.id;
