...
import { generateCertificationRecommendations } from "./ai-certifications";
import { generateCareerPaths } from "./ai-career-paths";
import { getCareerAdvice, generateResumeSuggestions, generateFullResume, generateCoverLetter, generateCoverLetterSuggestions, generateInterviewQuestions, suggestCareerGoals, analyzeInterviewAnswer, generateRoleInsights } from "./openai";
...
export async function registerRoutes(app) {
    ...
    apiRouter.get("/health", async (req, res) => {
        try {
            ...
            const status = storageHealth.status === "healthy"
                ? "healthy"
    ...
    apiRouter.get("/public-reviews", (req, res) => {
        ...
        try {
            db.select({
                ...
                .where(and(eq(userReviews.isPublic, true), eq(userReviews.status, "approved")
    ...
    apiRouter.post("/review/submit", async (req, res) => {
        try {
            ...
            if (!rating || rating < 1 || rating > 5) {
                return res
                    .status(400)
                    .json({ message: "Valid rating between 1-5 is required" });
            }
    ...
    apiRouter.get("/admin/users", requireAdmin, async (req, res) => {
        try {
            ...
            const { data: usersData, error } = await supabaseAdmin
                .from("users")
                .select("id, username, name, email, user_type, university_id, created_at, subscription_plan, subscription_status, xp, level")
            ...
            const formattedUsers = (usersData || []).map((user) => ({
                ...
                accountStatus: user.subscription_status === "active" ? "active" : "inactive", // Map subscription status to account status
    ...
    apiRouter.get("/admin/users/stats", requireAdmin, async (req, res) => {
        try {
    ...
    try {
        ...
        if (!existingAdmin) {
            ...
            const hashedPassword = crypto
                .pbkdf2Sync(adminPwd, salt, 1000, 64, "sha512")
                .toString("hex");
        ...
        if (!existingUser) {
            ...
            const salt = crypto.randomBytes(16).toString("hex");
            const hashedPassword = crypto
                .pbkdf2Sync(userPwd, salt, 1000, 64, "sha512")
                .toString("hex");
            const securePassword = `${hashedPassword}.${salt}`;
            ...
            if (loginType) {
                if (loginType === "university" && user.userType === "regular") {
                    return res.status(403).json({
                        message: "Access denied. This account is not associated with a university."
                    });
                }
    ...
    apiRouter.post("/admin/create-staff", requireAdmin, async (req, res) => {
        try {
    ...
    apiRouter.post("/api/staff/register", async (req, res) => {
    ...
    apiRouter.post("/users/update-username", requireAuth, async (req, res) => {
        try {
    ...
    // User Routes
    apiRouter.get("/users/me", requireAuth, // Changed from requireLoginFallback to requireAuth
    async (req, res) => {
        try {
    ...
    apiRouter.post("/users/profile-image", async (req, res) => {
        try {
            ...
            if (req.body && req.body.imageDataUrl) {
                ...
                const { data: uploadData, error: uploadError } = await supabaseAdmin.storage.from("images").upload(filePathInBucket, buffer, { contentType: `image/${imageType}` });
                if (uploadError) {
                    console.error("Error uploading image to Supabase:", uploadError);
                    return res.status(500).json({ message: "Error uploading image" });
                }
                // Get public URL
                const { data: { publicUrl } } = supabaseAdmin.storage.from("images").getPublicUrl(filePathInBucket);
    ...
    apiRouter.put("/users/subscription", async (req, res) => {
        try {
            ...
            const { subscriptionPlan, subscriptionStatus, subscriptionCycle, stripeCustomerId, stripeSubscriptionId, subscriptionExpiresAt } = req.body;
    ...
    apiRouter.get("/users/statistics", async (req, res) => {
        try {
    ...
    apiRouter.get("/users/xp-history", async (req, res) => {
        try {
            // Get the current authenticated user
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    // Goal Routes
    apiRouter.get("/goals", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.post("/goals", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.put("/goals/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.delete("/goals/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const goalId = parseInt(id);
            if (isNaN(goalId)) {
            ...
            if (goal.userId !== user.id) {
                return res
                    .status(403)
                    .json({ message: "You don't have permission to delete this goal" });
            }
    ...
    apiRouter.post("/goals/suggest", async (req, res) => {
        try {
            ...
            const goalSuggestions = await suggestCareerGoals(currentPosition, desiredPosition, timeframe, skills);
    ...
    apiRouter.get("/skill-stacker", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.get("/skill-stacker/goal/:goalId", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
            ...
            if (!goal || goal.userId !== user.id) {
                return res
                    .status(403)
                    .json({ message: "Access denied or goal not found" });
            }
            ...
            res
                .status(500)
                .json({ message: "Error fetching skill stacker plans for goal" });
        }
    });
    apiRouter.get("/skill-stacker/:id", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
            ...
            if (!plan || plan.userId !== user.id) {
                return res
                    .status(403)
                    .json({ message: "Access denied or plan not found" });
            }
    ...
    apiRouter.post("/skill-stacker", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
            ...
            if (existingPlan) {
                return res
                    .status(409)
                    .json({ message: "A plan already exists for this goal and week" });
            }
            // Create the plan
    ...
    apiRouter.post("/skill-stacker/generate", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.put("/skill-stacker/:id", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.put("/skill-stacker/:id/task/:taskId", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.put("/skill-stacker/:id/complete", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.delete("/skill-stacker/:id", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    // Work History Routes
    apiRouter.get("/work-history", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/work-history", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.put("/work-history/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (item.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to update this work history item"
                });
            }
    ...
    apiRouter.delete("/work-history/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const itemId = parseInt(id);
            if (isNaN(itemId)) {
            ...
            if (item.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to delete this work history item"
                });
            }
    ...
    apiRouter.get("/education-history", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/education-history", requireAuth, async (req, res) => {
        try {
            ...
            res
                .status(500)
                .json({ message: "Error creating education history item" });
        }
    });
    apiRouter.put("/education-history/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const itemId = parseInt(id);
            if (isNaN(itemId)) {
                return res
                    .status(400)
                    .json({ message: "Invalid education history ID" });
            }
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
            ...
            if (!item) {
                return res
                    .status(404)
                    .json({ message: "Education history item not found" });
            }
            // Ensure the education history item belongs to the current user
            if (item.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to update this education history item"
                });
            }
            ...
            res
                .status(500)
                .json({ message: "Error updating education history item" });
        }
    });
    apiRouter.delete("/education-history/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const itemId = parseInt(id);
            if (isNaN(itemId)) {
            ...
            if (item.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to delete this education history item"
                });
            }
            ...
            res
                .status(500)
                .json({ message: "Error deleting education history item" });
        }
    });
    // Certification Routes
    apiRouter.get("/certifications", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
            ...
            const safeCertifications = certifications.map((item) => ({
                ...item,
                issueDate: item.issueDate instanceof Date
    ...
    apiRouter.post("/certifications", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.put("/certifications/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.patch("/certifications/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const certificationId = parseInt(id);
            if (isNaN(certificationId)) {
    ...
    apiRouter.delete("/certifications/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const certificationId = parseInt(id);
            if (isNaN(certificationId)) {
    ...
    apiRouter.get("/personal-achievements", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/personal-achievements", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.put("/personal-achievements/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.delete("/personal-achievements/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const achievementId = parseInt(id);
            if (isNaN(achievementId)) {
    ...
    // Resume Routes
    apiRouter.get("/resumes", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.get("/resumes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            ...
            const safeResume = {
                ...resume,
                createdAt: resume.createdAt instanceof Date
                    ? resume.createdAt.toISOString()
                    : resume.createdAt,
                updatedAt: resume.updatedAt instanceof Date
    ...
    apiRouter.post("/resumes", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.put("/resumes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const resumeId = parseInt(id);
            if (isNaN(resumeId)) {
    ...
    apiRouter.delete("/resumes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const resumeId = parseInt(id);
            if (isNaN(resumeId)) {
    ...
    apiRouter.post("/resumes/suggestions", async (req, res) => {
        try {
                ...
                if (userHistoryEntries && userHistoryEntries.length > 0) {
                    userWorkHistory = userHistoryEntries
                        ...
                        const duration = job.currentJob
                            ...
                            : `${new Date(job.startDate).toLocaleDateString()} - ${job.endDate
    ...
    apiRouter.post("/resumes/generate", async (req, res) => {
        try {
            const { workHistory, jobDescription } = req.body;
            if (!jobDescription) {
            ...
            res
                .status(500)
                .json({ message: `Error generating resume: ${error.message}` });
        }
    });
    // Resume file upload endpoint
    apiRouter.post("/resumes/upload", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/resumes/extract-text", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.post("/resumes/analyze", requireAuth, async (req, res) => {
        try {
    ...
    // Cover Letter Routes
    apiRouter.get("/cover-letters", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.get("/cover-letters/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.put("/cover-letters/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const letterId = parseInt(id);
            if (isNaN(letterId)) {
    ...
    apiRouter.delete("/cover-letters/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const letterId = parseInt(id);
            if (isNaN(letterId)) {
    ...
    apiRouter.post("/cover-letters/analyze", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.post("/cover-letters/clean-optimized", async (req, res) => {
        const { optimizedLetter } = req.body;
        if (!optimizedLetter) {
            return res
                .status(400)
                .json({ error: "Missing optimized letter content" });
        }
        try {
    ...
    apiRouter.post("/api/strip-optimized-cover-letter", async (req, res) => {
        const { optimizedLetter } = req.body;
        if (!optimizedLetter) {
            return res
                .status(400)
    ...
    apiRouter.post("/save-cleaned-cover-letter", async (req, res) => {
        try {
...
`;
            try {
                ...
                if (cleanedBody.length > 200) {
                    const firstHundredChars = cleanedBody
                        .substring(0, 100)
                        .toLowerCase();
                    const lastHundredChars = cleanedBody
            ...
            return res
                .status(500)
                .json({ error: "Failed to clean cover letter for saving" });
        }
    });
    apiRouter.post("/cover-letters/generate", async (req, res) => {
        try {
            ...
            if (type === "suggestions") {
                if (!jobDescription) {
                    return res
                        .status(400)
                        .json({ message: "Job description is required for suggestions" });
                }
            }
            else {
            ...
            if (type === "suggestions") {
                ...
                const suggestions = await generateCoverLetterSuggestions(jobTitle || "Unspecified Position", companyName || "Unspecified Company", jobDescription, userExperience || "", userSkills || "");
                res.status(200).json({ suggestions });
            }
            else {
                ...
                if (req.userId) {
                    try {
                        // 1. Get work history
                        const workHistoryEntries = await storage.getWorkHistory(req.userId);
                        ...
                        const educationEntries = await storage.getEducationHistory(req.userId);
                        if (educationEntries && educationEntries.length > 0) {
                            formattedEducation = educationEntries
                                .map((edu) => {
                                const duration = edu.current
                                    ? `${new Date(edu.startDate).toLocaleDateString()} - Present`
                                    : `${new Date(edu.startDate).toLocaleDateString()} - ${edu.endDate
                        ...
                        if (skillEntries && skillEntries.length > 0) {
                            formattedSkills = skillEntries.map((skill) => skill.name);
                        }
                ...
                try {
                    const coverLetter = await generateCoverLetter(jobTitle || "", companyName || "", jobDescription, careerData, userProfile // Pass the user profile data including name, email, etc.
                    );
    ...
    // Interview Routes
    apiRouter.get("/api/interview/questions", async (req, res) => {
        try {
            const { category } = req.query;
            const questions = await storage.getInterviewQuestions(category);
    ...
    apiRouter.post("/api/interview/practice", requireAuth, async (req, res) => {
        try {
            ...
            if (error instanceof z.ZodError) {
                return res
                    .status(400)
                    .json({ message: "Invalid practice data", errors: error.errors });
            }
            res.status(500).json({ message: "Error saving interview practice" });
        }
    });
    apiRouter.get("/api/interview/practice-history", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/api/interview/generate-questions", async (req, res) => {
        try {
            const { jobTitle, skills } = req.body;
            if (!jobTitle || !skills) {
                return res
                    .status(400)
                    .json({ message: "Job title and skills are required" });
            }
            ...
            res
                .status(500)
                .json({ message: "Error generating interview questions" });
        }
    });
    apiRouter.post("/api/interview/analyze-answer", async (req, res) => {
        try {
    ...
    apiRouter.get("/achievements/user", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.get("/mentor-chat/conversations", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/mentor-chat/conversations", requireAuth, async (req, res) => {
        try {
            ...
            switch (mentorPersona) {
                case "interviewer":
                    welcomeMessage =
                        "Hi there! I'm your interview specialist. I can help you prepare for interviews, practice responses, and develop strategies to impress potential employers. What would you like to discuss today?";
                    break;
                case "industry_expert":
                    welcomeMessage =
                        "Hello! I'm your industry advisor. I can provide insights about industry trends, company cultures, and market demands. What industry questions can I help with today?";
                    break;
                case "resume_expert":
                    welcomeMessage =
                        "Welcome! I specialize in resume and cover letter optimization. I can help you craft compelling documents that highlight your strengths and catch recruiters' attention. How can I assist with your application materials?";
                    break;
                default:
                    welcomeMessage =
                        "Hello! I'm your career coach and mentor. I'm here to guide you through your professional journey with personalized advice and actionable strategies. What's on your mind today?";
            }
    ...
    apiRouter.get("/mentor-chat/conversations/:id/messages", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.post("/mentor-chat/conversations/:id/messages", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const conversationId = parseInt(id);
            if (isNaN(conversationId)) {
    ...
    // AI Coach Routes
    apiRouter.get("/ai-coach/conversations", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/ai-coach/conversations", requireAuth, async (req, res) => {
        try {
            ...
            const conversation = await storage.createAiCoachConversation(user.id, {
                title
            });
    ...
    apiRouter.get("/ai-coach/conversations/:id/messages", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const conversationId = parseInt(id);
            if (isNaN(conversationId)) {
    ...
    apiRouter.post("/ai-coach/conversations/:id/messages", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const conversationId = parseInt(id);
            if (isNaN(conversationId)) {
    ...
    apiRouter.post("/generate-role-details", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.post("/ai-coach/generate-response", requireAuth, async (req, res) => {
        try {
            ...
            const validMessages = formattedMessages.filter((msg) => msg && typeof msg.content === "string" && msg.content.trim() !== "");
    ...
    apiRouter.get("/interview/processes", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/interview/processes", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.get("/interview/processes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (!process) {
                return res
                    .status(404)
                    .json({ message: "Interview process not found" });
            }
            // Ensure the process belongs to the current user
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to access this interview process"
                });
            }
    ...
    apiRouter.put("/api/interview/processes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const processId = parseInt(id);
            if (isNaN(processId)) {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to update this interview process"
                });
            }
    ...
    apiRouter.delete("/api/interview/processes/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const processId = parseInt(id);
            if (isNaN(processId)) {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to delete this interview process"
                });
            }
    ...
    apiRouter.get("/api/interview/processes/:processId/stages", requireAuth, async (req, res) => {
        try {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to access stages for this interview process"
                });
            }
    ...
    apiRouter.post("/api/interview/processes/:processId/stages", requireAuth, async (req, res) => {
        try {
            const { processId } = req.params;
            const processIdNum = parseInt(processId);
    ...
    apiRouter.put("/api/interview/stages/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.delete("/api/interview/stages/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const stageId = parseInt(id);
            if (isNaN(stageId)) {
    ...
    apiRouter.get("/api/interview/processes/:processId/followups", requireAuth, async (req, res) => {
        try {
            const { processId } = req.params;
            const processIdNum = parseInt(processId);
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to access followups for this interview process"
                });
            }
            let stageIdNum = undefined;
    ...
    apiRouter.post("/api/interview/processes/:processId/followups", requireAuth, async (req, res) => {
        try {
            const { processId } = req.params;
            const processIdNum = parseInt(processId);
            if (isNaN(processIdNum)) {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to add followups to this interview process"
                });
            }
    ...
    apiRouter.put("/api/interview/followup-actions/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.put("/api/interview/followup-actions/:id/complete", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const actionId = parseInt(id);
            if (isNaN(actionId)) {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to complete this followup action"
                });
            }
    ...
    apiRouter.put("/api/interview/followup-actions/:id/uncomplete", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const actionId = parseInt(id);
            if (isNaN(actionId)) {
            ...
            if (process.userId !== user.id) {
                return res.status(403).json({
                    message: "You don't have permission to uncomplete this followup action"
                });
            }
    ...
    apiRouter.get("/api/interview/followup-actions", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.delete("/api/interview/followup-actions/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const actionId = parseInt(id);
            if (isNaN(actionId)) {
    ...
    apiRouter.post("/api/support", async (req, res) => {
        try {
    ...
    apiRouter.post("/api/in-app/support", requireAuth, async (req, res) => {
        try {
            const { issueType, subject, description, priority, attachmentUrl, source = "in-app", name, email, universityName, department, contactPerson } = req.body;
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.get("/api/admin/support-tickets", requireAdmin, async (req, res) => {
        try {
            const { source, issueType, status, search, universityName } = req.query;
            // Build query filters
            const filters = {};
            if (source && source !== "all")
                filters.source = source.toString();
            if (issueType && issueType !== "all")
                filters.issueType = issueType.toString();
            if (status && status !== "all")
                filters.status = status.toString();
            if (universityName)
                filters.universityName = universityName.toString();
            ...
            if (search) {
                ...
                tickets = tickets.filter((ticket) => {
                    // Standard fields
                    const inUserEmail = ticket.userEmail &&
                        ticket.userEmail.toLowerCase().includes(searchStr);
                    const inDescription = ticket.description
                        .toLowerCase()
                        .includes(searchStr);
                    // University-specific fields
                    const inUniversityName = ticket.universityName &&
                        ticket.universityName.toLowerCase().includes(searchStr);
                    const inDepartment = ticket.department &&
                        ticket.department.toLowerCase().includes(searchStr);
                    const inContactPerson = ticket.contactPerson &&
                        ticket.contactPerson.toLowerCase().includes(searchStr);
                    return (inUserEmail ||
                        inDescription ||
                        inUniversityName ||
                        inDepartment ||
    ...
    apiRouter.get("/api/admin/support-tickets/:id", requireAdmin, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.put("/api/admin/support-tickets/:id", requireAdmin, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    apiRouter.get("/contact/messages", requireStaff, async (req, res) => {
        try {
    ...
    apiRouter.put("/contact/messages/:id/read", requireStaff, async (req, res) => {
        try {
            ...
            res
                .status(500)
                .json({ message: "Failed to mark contact message as read" });
        }
    });
    apiRouter.put("/contact/messages/:id/archive", requireStaff, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.post("/api/theme", requireAuth, async (req, res) => {
        try {
            ...
            res
                .status(200)
                .json({ message: "Theme updated successfully", theme: themeData });
        }
        catch (error) {
    ...
    apiRouter.post("/payments/create-payment-intent", async (req, res) => {
        try {
    ...
    apiRouter.post("/payments/create-subscription", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/payments/webhook", express.raw({ type: "application/json" }), async (req, res) => {
        try {
    ...
    apiRouter.get("/payments/payment-methods", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.get("/payment-methods", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.post("/payments/create-setup-intent", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/subscription/upgrade", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/payments/cancel-subscription", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/subscription/cancel", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/auth/send-verification-email", requireAuth, async (req, res) => {
        try {
            // Get current user from session
            const user = await getCurrentUser(req);
            if (!user) {
    ...
    apiRouter.post("/auth/send-email-change-verification", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.get("/auth/verify-email-change", async (req, res) => {
        try {
            const { token } = req.query;
            if (!token || typeof token !== "string") {
                return res.status(400).json({ error: "Missing or invalid token" });
    ...
    apiRouter.get("/recommendations/daily", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
    ...
    const setupRecommendationsScheduler = async () => {
        ...
        const scheduleNextRefresh = () => {
            const now = new Date();
            const nextRefresh = new Date(now.getFullYear(), now.getMonth(), now.getDate() +
    ...
    apiRouter.post("/recommendations/:id/complete", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Authentication required" });
            ...
            if (recommendation.userId !== user.id) {
                return res.status(403).json({
                    message: "Access denied. You can only update your own recommendations."
                });
            }
            ...
            if (!completedRecommendation) {
                return res
                    .status(500)
                    .json({ message: "Failed to complete recommendation" });
            }
    ...
    apiRouter.get("/job-listings", requireAuth, async (req, res) => {
        try {
    ...
    apiRouter.get("/job-listings/:id", requireAuth, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.post("/job-listings", requireAuth, requireAdmin, async (req, res) => {
        try {
    ...
    apiRouter.put("/job-listings/:id", requireAuth, requireAdmin, async (req, res) => {
        try {
    ...
    apiRouter.delete("/job-listings/:id", requireAuth, requireAdmin, async (req, res) => {
        try {
    ...
    apiRouter.get("/job-applications", requireAuth, async (req, res) => {
        try {
            const user = await getCurrentUser(req);
            if (!user) {
                return res.status(401).json({ message: "Unauthorized" });
    ...
    apiRouter.get("/applications/:id", requireAuth, validateUserAccess, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.post("/applications", requireAuth, async (req, res) => {
        try {
            ...
            const wizardSteps = await Promise.all(defaultSteps.map((step) => storage.createApplicationWizardStep(application.id, step)));
            res.status(201).json({
                application,
                steps: wizardSteps
            });
    ...
    apiRouter.put("/applications/:id", requireAuth, validateUserAccess, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.post("/applications/:id/submit", requireAuth, validateUserAccess, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.delete("/applications/:id", requireAuth, validateUserAccess, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.get("/applications/:applicationId/steps", requireAuth, validateUserAccess, async (req, res) => {
        try {
    ...
    apiRouter.get("/api/application-steps/:id", requireAuth, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.put("/api/application-steps/:id", requireAuth, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
    ...
    apiRouter.post("/api/application-steps/:id/complete", requireAuth, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            // First get the step to verify user access
            const step = await storage.getApplicationWizardStep(id);
    ...
    apiRouter.post("/career-certifications", async (req, res) => {
        try {
            ...
            res
                .status(500)
                .json({ message: "Failed to generate certification recommendations" });
        }
    });
    // API endpoint for generating personalized career paths based on user profile
    apiRouter.post("/api/career-paths/generate", requireAuth, async (req, res) => {
        try {
