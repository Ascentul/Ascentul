...
export function registerAICoachRoutes(app) {
    // Get all conversations for the current user
    app.get("/api/ai-coach/conversations", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
            // Get conversations from the storage
            const userConversations = await storage.getAiCoachConversations(req.userId);
    ...
    app.post("/api/ai-coach/conversations", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
            ...
            const newConversation = await storage.createAiCoachConversation(req.userId, {
                title
            });
    ...
    app.get("/api/ai-coach/conversations/:id/messages", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
    ...
    app.post("/api/ai-coach/conversations/:id/messages", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
    ...
    app.post("/api/ai-coach/generate-response", async (req, res) => {
        try {
            if (!req.userId) {
                return res.status(401).json({ error: "Not authenticated" });
            }
