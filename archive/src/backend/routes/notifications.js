...
router.post('/:id/read', verifySupabaseToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/read-all', verifySupabaseToken, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/mark-read', verifySupabaseToken, async (req, res) => {
    try {
        const userId = req.user.id;
        await markNotificationsRead(userId);
        res.json({ success: true });
