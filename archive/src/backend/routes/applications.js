...
import { z } from 'zod';
export function registerApplicationRoutes(app, storage) {
    ...
    app.get('/api/applications', requireLoginFallback, async (req, res) => {
        try {
    ...
    app.get('/api/job-applications', requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId;
            const applications = await storage.getJobApplications(userId);
            res.json(applications);
    ...
    app.post('/api/applications', requireLoginFallback, async (req, res) => {
        try {
            const userId = req.userId;
            // Validate the input
    ...
    app.post('/api/applications/:id/submit', requireLoginFallback, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const userId = req.userId;
    ...
    app.delete('/api/applications/:id', requireLoginFallback, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const userId = req.userId;
            // Check if the application exists and belongs to the user
    ...
    app.get('/api/applications/:id/steps', requireLoginFallback, async (req, res) => {
        try {
    ...
    app.put('/api/applications/steps/:stepId', requireLoginFallback, async (req, res) => {
        try {
    ...
    app.post('/api/applications/steps/:stepId/complete', requireLoginFallback, async (req, res) => {
        try {
            const stepId = parseInt(req.params.stepId);
            const userId = req.userId;
            // Get the step
