...
export function InterviewCountdownCard() {
    ...
    const fetchNextInterview = async () => {
        ...
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (!key)
                    continue;
                if (!key.includes('mockStages_') && !key.includes('mockInterviewStages_'))
                    continue;
                try {
                ...
                if (nextStage.applicationId && (!company || !position)) {
                    ...
                    if (applicationsJson) {
                        try {
                            ...
                            const application = applications.find((app) => app.id === nextStage.applicationId);
                            if (application) {
    ...
    return (_jsx(Link, { href: "/job-applications?filter=interviewing", children: _jsx(Card, { className: "cursor-pointer hover:shadow-md transition-shadow", children: _jsx(CardContent, { className: "p-4", children: _jsxs("div", { className: "flex items-center", children: [_jsx("div", { className: "flex-shrink-0 p-3 rounded-full bg-blue-500/20", children: _jsx(Calendar, { className: "h-5 w-5 text-blue-500" }) }), _jsxs("div", { className: "ml-4", children: [_jsx("h3", { className: "text-neutral-500 text-sm", children: "Next Interview" }), _jsx("p", { className: "text-2xl font-semibold", children: getDisplayText() })] })] }) }) }) }));
}
