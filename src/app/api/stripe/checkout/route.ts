  ...
  try {
    ...
    if (!customerId) {
      ...
      if (existingCustomers.data.length > 0) {
        ...
        console.log(
          `[Stripe Checkout] Found existing Stripe customer ${customerId} for user ${userId}`,
        );
        ...
        try {
          ...
          console.error(
            `[Stripe Checkout] Failed to sync existing Stripe customer: ${customerId} for user ${userId}`,
          );
        }
      } else {
        ...
        try {
          ...
          console.error(
            `[Stripe Checkout] Orphaned Stripe customer: ${customerId} for user ${userId}`,
          );
