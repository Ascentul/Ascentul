  ...
  try {
    ...
    if (isPdf) {
      try {
        ...
        return NextResponse.json({ text, info: { pages: result.numpages, version: result.version, type: 'pdf' } })
      } catch (pdfError: any) {
        console.error('PDF parse error:', pdfError)
        // Fallback: return basic info about the file
        return NextResponse.json({ 
          text: '', 
          info: { type: 'pdf', error: 'Could not extract text from this PDF. It may be image-based or corrupted.' },
