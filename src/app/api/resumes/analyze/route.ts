...
const highlightChecks: Array<{ keywords: string[]; message: string }> = [
  ...
  {
    keywords: ["implement", "implemented", "launch", "launched", "build", "built", "developed", "deliver", "delivered"],
...
function simpleAnalyze(resumeText: string, jobDescription: string) {
  ...
  const resumePhrases = new Set(sanitizeTokens(extractPhrases(res, 2).concat(extractPhrases(res, 3))))
  ...
  if (techTerms.length > 0) {
    suggestions.push(`Highlight your experience with ${techTerms.slice(0, 3).join(', ')} in your skills section or project descriptions.`)
  }
  ...
  if (leadershipTerms.length > 0) {
    suggestions.push(`Add examples showing your leadership or teamwork experience to demonstrate collaborative skills.`)
  }
  ...
  if (domainTerms.length > 0) {
    suggestions.push(`If you have ${domainTerms[0]} experience, emphasize it prominently to show domain knowledge.`)
  }
  ...
  if (certTerms.length > 0) {
    suggestions.push(`Include any relevant certifications or educational credentials in a dedicated section.`)
  }

  // General suggestions for remaining gaps
  const remainingGaps = gaps.filter((g) =>
    !techTerms.includes(g) && !leadershipTerms.includes(g) &&
    !domainTerms.includes(g) && !certTerms.includes(g)
  ).slice(0, 3)

  if (remainingGaps.length > 0) {
    suggestions.push(`Consider weaving in keywords like "${remainingGaps.join('", "')}" where they naturally fit your experience.`)
  }

  // Always add quantification suggestion
  if (score < 70) {
    suggestions.push(`Use specific metrics and numbers to quantify your achievements (e.g., "increased sales by 25%", "managed team of 5").`)
  }
...
Keep suggestions practical, encouraging, and human.`
        ...
        return NextResponse.json({
          ...
          strengthHighlights: Array.isArray(parsed.strengthHighlights) && parsed.strengthHighlights.length > 0
