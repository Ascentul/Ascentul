...
export async function POST(request: NextRequest) {
  try {
    ...
    const envCountry = typeof process.env.ADZUNA_COUNTRY === 'string' ? process.env.ADZUNA_COUNTRY.trim().toLowerCase() : ''
    ...
    // Map job type
    if (jobType === 'Full-time') params.set('contract_time', 'full_time')
    else if (jobType === 'Part-time') params.set('contract_time', 'part_time')
    else if (jobType === 'Contract') params.set('contract_type', 'contract')
    // Internship not directly supported; could filter by category or keywords
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 || looksHtml) {
        ...
        if (retryRes.ok) {
          ...
          try { parsedRetry = JSON.parse(raw) } catch {}
          ...
          return NextResponse.json({ error: 'Adzuna request failed', details: parsedRetry || raw, meta }, { status: retryRes.status })
        }
      } else {
        let parsed: any = null
        ...
        return NextResponse.json({ error: 'Adzuna request failed', details: parsed || raw, meta }, { status: res.status })
      }
    }
    ...
    const jobs = (data.results || []).map((r: any) => {
      ...
      return {
        ...
        type: r.contract_time ? (r.contract_time === 'full_time' ? 'Full-time' : 'Part-time') : (jobType || '—'),
        experience: experienceLevel || '—',
        description: r.description ? String(r.description).replace(/<[^>]+>/g, '').slice(0, 220) + '…' : '—',
        salary,
        url: r.redirect_url,
