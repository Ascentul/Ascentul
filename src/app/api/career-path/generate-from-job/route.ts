...
const inferLevelFromTitle = (value: string): StageLevel => {
  ...
  if (/chief|c[eoif]o\b|vice president|vp\b|president|founder|partner/.test(lower))
    return 'executive';
  ...
  if (/\b(junior|jr\.?|assistant|associate|intern|entry)\b/.test(lower)) return 'entry';
  return 'mid';
};

const dedupeTitle = (candidate: string, ctx: TemplateContext, fallback: string) =>
  candidate.trim().toLowerCase() === ctx.targetRole.toLowerCase() ? fallback : candidate;
...
const softwareTemplate: CareerPathTemplate = {
      ...
      if (ctx.isExecutive) {
        ...
        switch (ctx.targetLevel) {
          ...
          case 'mid':
            ...
            break;
          case 'entry':
            baseTitle = spec.label === 'Software Engineer' ? 'Software Developer I' : spec.early;
            break;
          default:
  ...
  targetStage: (ctx) => {
    ...
    const salary = ctx.isExecutive
      ? '$180,000 - $230,000'
      : ctx.isManager
        ? '$140,000 - $190,000'
        : '$120,000 - $165,000';
    const experience = ctx.isExecutive ? '12+ years' : ctx.isManager ? '7-10 years' : '4-7 years';
    return {
...
const dataTemplate: CareerPathTemplate = {
  ...
  stages: [
    () => ({
      ...
      description: 'Clean datasets, build dashboards, and answer directional business questions.',
      growthPotential: 'high',
      icon: 'graduation',
    }),
    (ctx) => ({
    ...
    (ctx) => {
      ...
      const fallback = ctx.isExecutive ? 'Analytics Program Lead' : 'Senior Data Specialist';
      const title = dedupeTitle(baseTitle, ctx, fallback);
      const level: StageLevel = ctx.isExecutive
        ? 'lead'
      ...
      const salaryRange =
        level === 'mid'
          ...
          : level === 'senior'
            ...
            : '$130,000 - $175,000';
      const yearsExperience =
        level === 'mid' ? '3-5 years' : level === 'senior' ? '5-7 years' : '7-10 years';
      return {
        title,
        level,
        salaryRange,
...
const productTemplate: CareerPathTemplate = {
  ...
  stages: [
    ...
    (ctx) => {
      ...
      const fallback = ctx.isExecutive ? 'Product Portfolio Lead' : 'Product Strategist';
      const title = dedupeTitle(baseTitle, ctx, fallback);
  ...
  targetStage: (ctx) => {
    const salary = ctx.isExecutive
      ...
      : ctx.isManager
        ? '$140,000 - $195,000'
        : '$110,000 - $150,000';
    const level = ctx.isExecutive ? 'executive' : ctx.isManager ? 'lead' : ctx.targetLevel || 'mid';
    const experience = ctx.isExecutive ? '12+ years' : ctx.isManager ? '7-10 years' : '4-7 years';
    return {
      title: ctx.targetRole,
      level,
  ...
  targetStage: (ctx) => {
    const salary = ctx.isExecutive
      ...
      : ctx.isManager
        ? '$140,000 - $190,000'
        : '$110,000 - $150,000';
    const level = ctx.isExecutive
      ? 'executive'
      : ctx.isManager
        ? 'lead'
        : ctx.targetLevel || 'senior';
    const experience = ctx.isExecutive ? '12+ years' : ctx.isManager ? '7-10 years' : '5-8 years';
    return {
      title: ctx.targetRole,
      level,
      salaryRange: salary,
...
const evaluatePathQuality = (
  ...
  const missingDescriptions = nodes.some(
    (node: any) =>
      !node?.description ||
      String(node.description).trim().length < QUALITY_THRESHOLDS.MIN_DESCRIPTION_LENGTH,
  );
  ...
  const minKeywordMatches =
    typeof template.minKeywordMatches === 'number'
      ? Math.max(
          1,
          Math.floor(template.minKeywordMatches * QUALITY_THRESHOLDS.KEYWORD_REDUCTION_FACTOR),
        )
...
const normalizeOpenAIPath = (rawPath: any, ctx: TemplateContext, index: number): CareerPath => {
    ...
    return {
      ...
      growthPotential:
        ...
        node?.growthPotential === 'high'
          ...
          : 'high',
      icon: typeof node?.icon === 'string' && node.icon.trim() ? node.icon.trim() : fallbackIcon,
    };
  });
...
const buildProfileGuidancePath = (
  ...
  const hasDomainSkills =
    skills.length > 0 &&
    skills.some((skill) => domainKeywords.some((keyword) => skill.includes(keyword)));
  ...
  if (!hasDomainWorkHistory) {
    pushTask({
      ...
      description: `Open Career Profile > Work History and add accomplishment-driven bullets that highlight ${displayDomain.toLowerCase()} impact aligning with ${ctx.targetRole}.`,
      icon: 'book',
  ...
  if (!hasDomainSkills || skills.length < 5) {
    ...
    pushTask({
      ...
      description: `Update Career Profile > Skills with the capabilities executives expect for ${ctx.targetRole} (e.g., ${recommended.join(', ')}).`,
      icon: 'layers',
      action_url: '/career-profile',
    });
  }
  ...
  if (!hasCareerGoal) {
    pushTask({
      ...
      description: `Head to Goals > Add Goal and describe why you are pursuing ${ctx.targetRole}. This steers the Career Path Explorer toward executive-ready steps.`,
      icon: 'linechart',
      action_url: '/goals',
    });
  }
      ...
      for (const variant of promptVariants) {
        for (const model of models) {
          ...
          try {
            ...
            const mapperResult = mapAiCareerPathToUI(parsed.paths[0], promptContext.targetRole);
