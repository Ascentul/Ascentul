...
function AdminDashboardPage() {
  ...
  const [activeView, setActiveView] = React.useState<
    'system' | 'universities' | 'users' | 'revenue'
  >('system');
  const [activeAnalyticsTab, setActiveAnalyticsTab] = React.useState<
    'overview' | 'analytics' | 'universities' | 'users' | 'system'
  >('overview');
  ...
  const universityAnalytics = useQuery(
    ...
    clerkLoaded && canAccess && clerkUser?.id && activeAnalyticsTab === 'universities'
  ...
  const revenueData = useQuery(
    ...
    clerkLoaded &&
      canAccess &&
      clerkUser?.id &&
      (activeView === 'revenue' || activeAnalyticsTab === 'users')
      ? { clerkId: clerkUser.id }
      : 'skip',
  );
  ...
  const users = useQuery(
    ...
    clerkLoaded && canAccess && clerkUser?.id && activeAnalyticsTab === 'users'
      ? { clerkId: clerkUser.id, limit: 50 }
      : 'skip',
  );
  ...
  const mauTrends = useMemo(
    () => universityAnalytics?.mauTrends || [],
    [universityAnalytics?.mauTrends],
  );
  ...
  const avgLicenseUtilization = useMemo(
    () =>
      universityData.length > 0
        ? Math.round(
            universityData.reduce((sum, uni) => sum + ((uni as any).licenseUtilization || 0), 0) /
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">
                You need the <span className="font-semibold text-red-600">super_admin</span> role to
                access this page.
              </p>
            </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-yellow-800">
                      Your Clerk role ({clerkRole || 'none'}) doesn't match your database role (
                      {convexRole || 'none'}).
                    </p>
                    <p className="text-sm text-yellow-800">
                      Authorization checks always use your <strong>Clerk role</strong>, which is the
                      source of truth. The database role is only used for display purposes.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
                    ...
                    <li>
                      The admin must set your role in <strong>Clerk Dashboard</strong> under your
                      user's Public Metadata
                    </li>
                    <li>
                      Set:{' '}
                      <code className="bg-white px-1 py-0.5 rounded">{'role: "super_admin"'}</code>
                    </li>
  ...
  return (
    <AdminDashboardContent
      ...
      revenueDataLoading={
        (activeView === 'revenue' || activeAnalyticsTab === 'users') && !revenueData
      }
...
const AdminDashboardContent = React.memo(function AdminDashboardContent({
  ...
  setActiveAnalyticsTab: (
    tab: 'overview' | 'analytics' | 'universities' | 'users' | 'system',
  ) => void;
  users: any;
  revenueData: any;
  supportMetrics: any;
    ...
    <div className="space-y-4 min-w-0">
      <div className="w-full min-w-0 rounded-3xl bg-white p-6 shadow-sm space-y-6">
        ...
        <Tabs
          ...
          className="space-y-6"
          onValueChange={(value) => setActiveAnalyticsTab(value as any)}
        >
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="text-sm">
                            <>
                              New user <strong>{activity.user}</strong> registered (
                              {activity.university})
                            </>
                          </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-blue-900">
                        Connect to Vercel Analytics or Convex Dashboard
                      </h4>
                      <p className="text-sm text-blue-700 mt-1">
                        For real-time system monitoring, connect to{' '}
                        <a
                        ...
                        for web vitals and performance metrics, or use the{' '}
                        <a
                        ...
                        for database metrics. Alternatively, integrate with Datadog or New Relic for
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Configure monitoring services for real-time metrics
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Track API response times, error rates, and system health
                          </p>
                        </div>
                      ...
                      <div className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Monitor platform availability and downtime
                          </p>
                        </div>
                        <Badge variant="outline">Not Configured</Badge>
                      </div>
                      <div className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Monitor database load, query performance, and storage
                          </p>
                        </div>
                        <Badge variant="outline">Not Configured</Badge>
                      </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{avgLicenseUtilization}%</div>
                        <p className="text-xs text-muted-foreground mt-1">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Percentage of licenses activated (students & advisors)
                      </CardDescription>
                    </CardHeader>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Student engagement trends over the last 6 months
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="h-96">
                      ...
                      {universityData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={universityData}>
                            ...
                            <Legend />
                            <Bar
                              dataKey="featureUsage.applications"
                              stackId="a"
                              fill="#4F46E5"
                              name="Applications"
                            />
                            <Bar
                              dataKey="featureUsage.resumes"
                              stackId="a"
                              fill="#10B981"
                              name="Resumes"
                            />
                            <Bar
                              dataKey="featureUsage.goals"
                              stackId="a"
                              fill="#F59E0B"
                              name="Goals"
                            />
                            <Bar
                              dataKey="featureUsage.projects"
                              stackId="a"
                              fill="#EC4899"
                              name="Projects"
                            />
                            <Bar
                              dataKey="featureUsage.coverLetters"
                              stackId="a"
                              fill="#8B5CF6"
                              name="Cover Letters"
                            />
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {(revenueData?.payingUsersCount ?? 0).toLocaleString()}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                          {systemStats.totalUsers > 0
                            ? Math.round(
                                ((revenueData?.payingUsersCount ?? 0) / systemStats.totalUsers) *
                                  100,
                              )
                            : 0}
                          % conversion
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 border rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              User registrations and logins over the past 7 days
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-4 border rounded-lg">
                        ...
                        <div className="flex items-center justify-between p-4 border rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              Detailed user behavior analytics require session tracking
                              implementation
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <p
                          className={`text-xs mt-1 ${(revenueData?.monthlyGrowthPercent ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                        >
                          {(revenueData?.monthlyGrowthPercent ?? 0) >= 0 ? '+' : ''}
                          {revenueData?.monthlyGrowthPercent ?? 0}% from last month
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {revenueData?.payingUsersCount ?? 0} paying users
                        </p>
                      </CardContent>
                    </Card>
                          ...
                          <PieChart>
                            ...
                            >
                              {[0, 1, 2].map((entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={['#4F46E5', '#10B981', '#F59E0B'][index]}
                                />
                              ))}
                            </Pie>
                            <Tooltip />
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {systemStats.totalUsers > 0
                      ? Math.round(
                          ((revenueData?.payingUsersCount ?? 0) / systemStats.totalUsers) * 100,
                        )
                      : 0}
                    % conversion rate
                  </p>
                    ...
                    <PieChart>
                      ...
                      >
                        {[0, 1, 2, 3].map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={['#4F46E5', '#10B981', '#F59E0B', '#EC4899'][index]}
                          />
                        ))}
                      </Pie>
                      <Tooltip />
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {users?.page &&
                    users.page.slice(0, 10).map((user: any) => (
                      <div
                        key={user._id}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Infrastructure monitoring (requires integration)
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                    <p className="text-sm text-muted-foreground">
                      System metrics require external monitoring integration (Datadog, New Relic,
                      etc.)
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-medium ${supportMetrics.openTickets > 0 ? 'text-orange-500' : 'text-muted-foreground'}`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium">{supportMetrics.resolvedTickets}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
