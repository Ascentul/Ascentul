...
class MockOpenAI {
  constructor() {
    console.warn(
      "⚠️ Using MockOpenAI - all OpenAI calls will return sample responses"
    )
  }
  ...
  chat = {
    completions: {
      create: async ({ messages, model, response_format }: any) => {
        console.log(
          `🔍 MockOpenAI: would have called gpt-4o-mini (was: ${model}) with ${messages.length} messages`
        )
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify({
                    message:
                      "This is mock data from MockOpenAI. The OPENAI_API_KEY is not set.",
          ...
          choices: [
            {
              message: {
                content:
                  "This is mock data from MockOpenAI. The OPENAI_API_KEY is not set. You are in development mode."
              }
            }
          ]
        }
