...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.use("/reviews/*", (req, res, next) => {
    console.log(
      `[DEBUG REVIEWS] Requested path: ${req.originalUrl}, Method: ${req.method}`
    )
    next()
  })
  ...
  apiRouter.get("/reviews/public", (req, res) => {
    ...
    try {
      db.select()
        ...
        .then((reviews) => {
          console.log(
            `Found ${reviews.length} public reviews (direct endpoint)`
          )
    ...
    try {
      db.select({
        ...
        .then((results) => {
          ...
          console.log(
            `Found ${formattedReviews.length} public approved reviews for WordPress`
          )
  ...
  apiRouter.get("/reviews/recent", (req, res) => {
    ...
    try {
      db.select()
        ...
        .then((reviews) => {
          console.log(
            `Found ${reviews.length} recent reviews (direct endpoint)`
          )
          return res.status(200).json({ reviews })
        })
        .catch((error) => {
  ...
  try {
    ...
    const existingUniversityAdmin = await storage.getUserByUsername(
      "university_admin"
    )
  ...
  apiRouter.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!result) {
          console.error(
            `Failed to delete goal with ID ${goalId} - storage.deleteGoal returned false`
          )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const existingPlan = await storage.getSkillStackerPlanByGoalAndWeek(
          result.data.goalId,
          result.data.week
        )
        if (existingPlan) {
        ...
        // Create the plan
        const newPlan = await storage.createSkillStackerPlan(
          user.id,
          result.data
        )
        ...
        try {
          ...
          const skillTasksMock = [
            ...
            },
            {
              ...
              description:
                "Complete 5-10 practice problems to reinforce concepts",
              estimatedHours: 2,
  ...
  apiRouter.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Update the plan
        const updatedPlan = await storage.updateSkillStackerPlan(
          planId,
          req.body
        )
  ...
  apiRouter.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedPlan = await storage.updateSkillStackerTaskStatus(
          planId,
          taskId,
          status,
          rating
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const workHistoryItem = await storage.createWorkHistoryItem(
          user.id,
          workHistoryData
        )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        const educationHistoryData = insertEducationHistorySchema.parse(
          req.body
        )
        ...
        const educationHistoryItem = await storage.createEducationHistoryItem(
          user.id,
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const achievementData = insertUserPersonalAchievementSchema.parse(
          req.body
        )
        const achievement = await storage.createUserPersonalAchievement(
          user.id,
          achievementData
        )
  ...
  apiRouter.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const achievement = await storage.getUserPersonalAchievement(
          achievementId
        )
  ...
  apiRouter.post("/resumes/generate", async (req: Request, res: Response) => {
    try {
      ...
      const resumeContent = await generateFullResume(
        userWorkHistory,
        jobDescription,
        userData
      )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const analysis = await openai.analyzeResumeForJob(
          resumeText,
          jobDescription
        )
        res.json(analysis)
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        console.log(
          "Received POST /api/cover-letters request with body:",
        ...
        try {
          ...
          console.error(
            "Validation error in cover letter creation:",
            validationError
          )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const analysis = await openai.analyzeCoverLetter(
          coverLetter,
          jobDescription
        )
        res.json(analysis)
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const cleanedLetterBody = await cleanOptimizedCoverLetter(
          optimizedLetter
        )
  ...
  apiRouter.post(
    "/api/strip-optimized-cover-letter",
    async (req: Request, res: Response) => {
      const { optimizedLetter } = req.body
...
- ANY company name or recipient lines including "${
...
`
        ...
        try {
          ...
          cleanedBody = cleanedBody
            ...
            .replace(
              /(new name test|CRM Analytics Analyst.*|vincentholm@gmail\.com|Grubhub|LinkedIn|\/\d{1,2}\/\d{4}|^\s*$)/gi,
              ""
            )
          ...
          if (cleanedBody.length > 200) {
            ...
            if (lastHundredChars.includes(firstHundredChars.substring(0, 50))) {
              console.log(
                "Duplicate content detected in final cover letter cleaning"
              )
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const questions = await storage.getInterviewQuestions(
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const practice = await storage.saveInterviewPractice(
          user.id,
          practiceData
        )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const conversation = await storage.createMentorChatConversation(
          user.id,
          {
            title,
        ...
        let welcomeMessage =
          "Hello! I'm your career mentor. How can I help you today?"
  ...
  apiRouter.get(
    "/mentor-chat/conversations/:id/messages",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params
        ...
        const conversation = await storage.getMentorChatConversation(
          conversationId
        )
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const userContext = {
          ...
          workHistory: workHistoryItems.map(
            (w) => `${w.position} at ${w.company}`
          )
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        await storage.addAiCoachMessage({
          ...
          isUser: false,
          message:
            "Hello! I'm your AI career coach. How can I help you today with your career goals?"
        })
  ...
  apiRouter.get(
    "/ai-coach/conversations/:id/messages",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const process = await storage.createInterviewProcess(
          user.id,
          processData
        )
  ...
  apiRouter.get(
    "/api/interview/processes/:processId/stages",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Adding stage to process ID: ${processId}, parsed as: ${processIdNum}`
        )
        ...
        console.log(
          `User ID: ${user.id}, looking for process with ID: ${processIdNum}`
        )
        ...
        const stage = await storage.createInterviewStage(
          processIdNum,
          stageData
        )
  ...
  apiRouter.get(
    "/api/interview/processes/:processId/followups",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { processId } = req.params
        ...
        const actions = await storage.getFollowupActions(
          processIdNum,
          stageIdNum
        )
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const action = await storage.createFollowupAction(
          processIdNum,
          actionData
        )
  ...
  apiRouter.put(
    "/api/interview/followup-actions/:id",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params
  ...
  apiRouter.put(
    "/api/interview/followup-actions/:id/complete",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params
  ...
  apiRouter.put(
    "/api/interview/followup-actions/:id/uncomplete",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params
  ...
  apiRouter.get(
    "/api/interview/followup-actions",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        // Get current user from session
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Processing recommendations request for user ID: ${user.id}`
        )
        ...
        if (shouldRefresh) {
          console.log(
            `Refresh requested. Clearing today's recommendations for user ${user.id}`
          )
        ...
        try {
          ...
          const recommendations = await storage.generateDailyRecommendations(
            user.id
          )
          console.log(
            `Successfully generated ${recommendations.length} recommendations for user ${user.id}`
          )
          return res.status(200).json(recommendations)
        } catch (genError: any) {
          console.error(
            `Specific error generating recommendations for user ${user.id}:`,
            genError
          )
  ...
  const setupRecommendationsScheduler = async () => {
    ...
    console.log(
      `Setting up daily recommendations refresh at ${RECOMMENDATIONS_REFRESH_TIME.hour}:${RECOMMENDATIONS_REFRESH_TIME.minute}:${RECOMMENDATIONS_REFRESH_TIME.second}`
    )
    ...
    const scheduleNextRefresh = () => {
      ...
      const nextRefresh = new Date(
        ...
        now.getMonth(),
        now.getDate() +
          (now.getHours() >= RECOMMENDATIONS_REFRESH_TIME.hour ? 1 : 0),
      ...
      console.log(
        `Next recommendations refresh scheduled for ${nextRefresh.toLocaleString()}`
      )
      ...
      setTimeout(async () => {
        try {
          console.log(
            `Running scheduled recommendations refresh at ${new Date().toLocaleString()}`
          )
          ...
          console.error(
            "Error during scheduled recommendations refresh:",
            error
          )
        } finally {
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const completedRecommendation = await storage.completeRecommendation(
          recommendationId
        )
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!role || !level || !Array.isArray(skills)) {
          return res.status(400).json({
            message:
              "Invalid request. Required fields: role (string), level (string), skills (array)"
          })
        }

        console.log(
          `Generating certification recommendations for ${role} (${level}) with ${skills.length} skills`
        )

        // Generate certification recommendations
        const certifications = await generateCertificationRecommendations(
          role,
          level,
          skills
        )
      ...
      try {
        ...
        if (!profileData) {
          ...
          console.log(
            ...
            } work history items, ${education?.length || 0} education items, ${
              skills?.length || 0
            } skills, ${certifications?.length || 0} certifications`
          )
        }
