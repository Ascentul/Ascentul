...
export async function generateCareerPaths(profileData) {
    try {
        ...
        const workHistoryText = workHistory
            .map((job) => `${job.title} at ${job.company} (${job.startDate} to ${job.endDate || "Present"}): ${job.description || ""}`)
            .join("\n");
        const educationText = education
            .map((edu) => `${edu.degree} in ${edu.fieldOfStudy} from ${edu.school} (${edu.graduationYear || "In progress"})`)
            .join("\n");
        const skillsText = skills
            .map((skill) => `${skill.name} (${skill.level || "intermediate"})`)
            .join(", ");
        const certificationsText = certifications
            .map((cert) => `${cert.name} from ${cert.issuingOrganization || "Unknown"}`)
            .join(", ");
