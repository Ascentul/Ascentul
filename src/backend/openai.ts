...
if (!process.env.OPENAI_API_KEY && process.env.NODE_ENV === "production") {
  ...
  console.warn(
    "⚠️ OPENAI_API_KEY not found — using mock responses in development mode."
  )
}
...
export async function analyzeInterviewAnswer(
  ...
  if (!answer || answer.trim() === "") {
    return {
      feedback:
        "You didn't provide an answer. Remember, even in practice, it's important to attempt a response to develop your skills.",
      strengthsScore: 1,
      improvementScore: 5,
      clarity: 1,
      relevance: 1,
...
${companyName ? `Company: ${companyName}` : ""}`
    ...
    return {
      feedback:
        "We couldn't analyze your answer at this time. Please try again later.",
      strengthsScore: 3,
      improvementScore: 3,
      clarity: 3,
      relevance: 3,
...
Be specific, actionable, and honest in your assessment. Focus on content, relevance, and persuasiveness rather than just format.`
    ...
    throw new Error(
      "An error occurred while analyzing the cover letter. Please try again later."
    )
  }
}
...
`
    ...
    const promptForOptimization =
      originalWorkHistoryItems && originalWorkHistoryItems.length > 0
        ? `
...
export async function generateCoverLetter(
  ...
  try {
    ...
    console.log(
      "Generating cover letter with user profile:",
      userProfile
        ? {
            id: userProfile.id,
    ...
    console.log(
      "Using userName for cover letter:",
      userName,
      "from user profile:",
      userProfile?.name
    )
