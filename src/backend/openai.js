...
export async function analyzeInterviewAnswer(question, answer, jobTitle, companyName, selectedModel) {
...
export async function getCareerAdvice(query, userContext, conversationHistory = []) {
    ...
    if (userContext) {
            ...
            userContext.workHistory.forEach((job, index) => {
                const duration = job.currentJob
                    ...
                    : `${new Date(job.startDate).toLocaleDateString()} - ${job.endDate
                        ...
                        : "Not specified"}`;
                systemPrompt += `${index + 1}. ${job.position} at ${job.company} (${duration})\n`;
                if (job.description) {
        ...
        else if (userContext.workHistory &&
            Array.isArray(userContext.workHistory) &&
            userContext.workHistory.length === 0) {
        ...
        else if (userContext.goals &&
            Array.isArray(userContext.goals) &&
        ...
        if (userContext.interviewProcesses &&
        ...
        else if (userContext.interviewProcesses &&
            Array.isArray(userContext.interviewProcesses) &&
            userContext.interviewProcesses.length === 0) {
        ...
        else if (userContext.achievements &&
            Array.isArray(userContext.achievements) &&
            userContext.achievements.length === 0) {
    ...
    try {
        ...
        return (response.choices[0].message.content ||
            "I'm not sure how to advise on that topic. Could you try rephrasing your question?");
    }
    catch (error) {
...
export async function generateResumeSuggestions(workHistory, jobDescription) {
    try {
...
export async function analyzeCoverLetter(coverLetter, jobDescription) {
    try {
...
export async function generateFullResume(workHistory, jobDescription, userData, originalWorkHistoryItems) {
    try {
...
export async function generateCoverLetterSuggestions(jobTitle, companyName, jobDescription, userExperience, userSkills) {
    try {
...
`;
        ...
        return (response.choices[0].message.content ||
            "Unable to generate cover letter suggestions at this time.");
    }
    catch (error) {
        console.error("OpenAI API error:", error);
        // Check for API key issues
...
export async function generateCoverLetter(jobTitle, companyName, jobDescription, careerData, userProfile // Optional user profile object with name, email, etc.
) {
    try {
...
export async function generateRoleInsights(currentRole, yearsExperience, industry, workHistory) {
    try {
        // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
        const response = await openaiInstance.chat.completions.create({
            model: "gpt-4o",
            messages: [
                {
                    role: "system",
                    content: "You are a career development AI that provides detailed insights and recommendations for career progression. " +
...
export async function optimizeCareerData(careerData, jobDescription) {
    try {
...
ID: ${item.id}`;
        ...
        const skillsText = careerData.skills && careerData.skills.length > 0
            ? careerData.skills.map((skill) => skill.name).join(", ")
            : "No skills provided";
...
export async function suggestCareerGoals(currentPosition, desiredPosition, timeframe, skills) {
    try {
                ...
                shortTerm: [
                    {
                        ...
                        description: "There's an issue with the OpenAI API key. Please contact the administrator to set up a valid API key."
                    }
                ],
                mediumTerm: [],
                longTerm: []
