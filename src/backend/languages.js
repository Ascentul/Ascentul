...
export function registerLanguagesRoutes(app, storage) {
    ...
    app.get("/api/languages/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // Update a language
    app.put("/api/languages/:id", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
            const languageId = parseInt(id);
            if (isNaN(languageId)) {
