...
class SettingsService {
  ...
  async getSettings(): Promise<PlatformSettings> {
    ...
    try {
      ...
      if (error) {
        console.warn(
          "Failed to fetch platform settings, using defaults:",
          error
        )
      ...
      if (data) {
        this.cachedSettings = {
          ...
          userRoles: data.user_roles || DEFAULT_SETTINGS.userRoles,
          ...
          xpSystem: data.xp_system || DEFAULT_SETTINGS.xpSystem,
      ...
      console.warn(
        "Exception fetching platform settings, using defaults:",
        error
      )
    }
