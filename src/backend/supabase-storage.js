...
export class SupabaseStorage {
    ...
    mapDatabaseUserToFrontend(dbUser) {
        if (!dbUser)
            return dbUser;
    ...
    async createSkill(skillData) {
        ...
        try {
            ...
            if (searchError && searchError.code !== "PGRST116") {
                ...
                throw new Error(`Failed to search for existing skill: ${searchError.message}`);
            }
            let skillId;
            if (existingSkill) {
            ...
            const { data: existingUserSkill, error: userSkillSearchError } = await supabase
                .from("user_skills")
                .select("*")
                .eq("user_id", userId)
                .eq("skill_id", skillId)
            ...
            if (userSkillSearchError && userSkillSearchError.code !== "PGRST116") {
                console.error("Error searching for existing user skill:", userSkillSearchError);
    ...
    async submitJobApplication(id, applied) {
    ...
    async createCoverLetter(userId, coverLetter) {
        try {
            const { data, error } = await supabase
                .from("cover_letters")
                .insert({
    ...
    async updateGoal(id, goalData) {
        try {
            ...
            const completingGoal = !currentGoal.completed && goalData.completed === true;
            // Map frontend field names to database column names
            const mappedData = {};
            if (goalData.title !== undefined)
                mappedData.title = goalData.title;
            ...
            if (goalData.status !== undefined)
                mappedData.status = goalData.status;
            if (goalData.dueDate !== undefined)
                mappedData.due_date = goalData.dueDate; // Map dueDate to due_date
            ...
            if (completingGoal ||
                ...
                await this.addUserXP(currentGoal.userId, currentGoal.xpReward ||
                    (await settingsService.getXpReward("goalCompletionReward")), "goals_completed", `Completed goal: ${currentGoal.title}`);
            }
    ...
    async getNetworkingContacts(userId, filters) {
        try {
            ...
            if (filters?.query) {
                query = query.or(`name.ilike.%${filters.query}%,company.ilike.%${filters.query}%,email.ilike.%${filters.query}%`);
            }
    ...
    async createContactInteraction(userId, contactId, interaction) {
        try {
            ...
            if (error) {
                ...
                throw new Error(`Failed to create contact interaction: ${error.message}`);
            }
            return data;
        }
        catch (exception) {
    ...
    async addAiCoachMessage(message) {
        try {
            ...
            if (message.isUser) {
                ...
                if (conversation) {
                    await this.addUserXP(conversation.user_id, 10, "ai_coach_interaction", "Interacted with AI Coach");
                }
            }
            return data;
        }
