...
export function registerCareerDataRoutes(app: Express, storage: IStorage) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `DEBUG: Found ${workHistoryItems.length} work history items for user ${userId}`
        )
        res.status(200).json({
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          console.log(
            `DEBUG: getUserSkills returned ${
              skills ? skills.length : "undefined"
            } skills for user ${userId}`
          )
          console.log(
            `DEBUG: Skills is type: ${typeof skills}, Array? ${Array.isArray(
              skills
            )}`
          )
          ...
          if (typeof storage.getCertifications === "function") {
            ...
            console.log(
              "getCertifications method not available in current storage implementation"
            )
        ...
        const serializedCertifications = certifications.map((item) => {
          ...
          try {
            ...
            if (serialized.createdAt) {
              ...
              if (
                Object.prototype.toString.call(serialized.createdAt) ===
                "[object Date]"
              ) {
            ...
            if (serialized.issueDate) {
              if (
                Object.prototype.toString.call(serialized.issueDate) ===
                "[object Date]"
              ) {
            ...
            if (serialized.expirationDate) {
              if (
                Object.prototype.toString.call(serialized.expirationDate) ===
                "[object Date]"
              ) {
        ...
        console.log(
          `Returning ${serializedWorkHistory.length} work history items, ${serializedEducationHistory.length} education items, ${serializedSkills.length} skills, and ${serializedCertifications.length} certifications`
        )
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          "⭐️ POST /api/career-data/work-history - Request received for user:",
          userId
        )
        ...
        console.log(
          `📋 User ${userId} has ${existingItems.length} existing work history items before creation`
        )
        ...
        console.log(
          `📋 User ${userId} now has ${updatedItems.length} work history items after creation`
        )

        // Debug all work history items to check that the newly created item is there
        updatedItems.forEach((item, index) => {
          console.log(
            `  Item ${index + 1}: ID=${item.id}, Company=${
              item.company
  ...
  app.put(
    "/api/career-data/work-history/:id",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        if (!req.userId) {
        ...
        const serializedItem = {
          ...updatedItem,
          startDate:
            updatedItem.startDate instanceof Date
              ? updatedItem.startDate.toISOString()
              : updatedItem.startDate,
          endDate:
            updatedItem.endDate instanceof Date
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          "⭐️ POST /api/career-data/education - Request received for user:",
          userId
        )
        console.log("📦 Raw request body:", JSON.stringify(req.body, null, 2))
        ...
        console.log(
          "Processing education form data with achievements:",
        ...
        console.log(
          `📋 User ${userId} has ${existingItems.length} existing education history items before creation`
        )

        const educationItem = await storage.createEducationHistoryItem(
          userId,
          formData
        )
        ...
        console.log(
          `📋 User ${userId} now has ${updatedItems.length} education history items after creation`
        )
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const userId = req.userId
        console.log(
          `DEBUG: Direct skills endpoint - Fetching skills for user ${userId}`
        )
        ...
        console.log(
          `DEBUG: Direct skills endpoint - Found ${skills.length} skills for user ${userId}`
        )
        if (skills.length > 0) {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `DEBUG: Creating skill for user ${userId}:`,
          JSON.stringify(skillData, null, 2)
        )

        const skill = await storage.createSkill(skillData)
        console.log(
          `DEBUG: Skill created successfully:`,
      ...
      try {
        ...
        try {
          if (updatedSkill.createdAt) {
            if (
              typeof updatedSkill.createdAt === "object" &&
              updatedSkill.createdAt !== null &&
              typeof (updatedSkill.createdAt as any).getTime === "function"
            ) {
              serializedSkill.createdAt = new Date(
                (updatedSkill.createdAt as any).getTime()
              ).toISOString()
            }
          }
        } catch (err) {
  ...
  app.post(
    "/api/career-data/certifications",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        if (!req.userId) {
        ...
        try {
          ...
          if (certification.createdAt) {
            if (
              typeof certification.createdAt === "object" &&
              certification.createdAt !== null &&
              typeof (certification.createdAt as any).getTime === "function"
            ) {
              serializedCertification.createdAt = new Date(
                (certification.createdAt as any).getTime()
              ).toISOString()
            }
          }
          ...
          if ("issueDate" in certification && certification.issueDate) {
            if (
              typeof certification.issueDate === "object" &&
              certification.issueDate !== null &&
              typeof (certification.issueDate as any).getTime === "function"
            ) {
              serializedCertification.issueDate = new Date(
                (certification.issueDate as any).getTime()
              ).toISOString()
            }
          }
          ...
          if (
            "expirationDate" in certification &&
            certification.expirationDate
          ) {
            if (
              typeof certification.expirationDate === "object" &&
              certification.expirationDate !== null &&
              typeof (certification.expirationDate as any).getTime ===
                "function"
            ) {
              serializedCertification.expirationDate = new Date(
                (certification.expirationDate as any).getTime()
              ).toISOString()
            }
          }
          ...
          console.error(
            "Error serializing certification dates:",
            serializationError
          )
  ...
  app.put(
    "/api/career-data/certifications/:id",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        if (!req.userId) {
        ...
        try {
          ...
          if (updatedCertification.createdAt) {
            if (
              typeof updatedCertification.createdAt === "object" &&
              updatedCertification.createdAt !== null &&
              typeof (updatedCertification.createdAt as any).getTime ===
                "function"
            ) {
              serializedCertification.createdAt = new Date(
                (updatedCertification.createdAt as any).getTime()
              ).toISOString()
            }
          }
          ...
          // Handle issueDate if it exists - using a safer type checking approach
          if (
            "issueDate" in updatedCertification &&
            updatedCertification.issueDate
          ) {
            if (
              typeof updatedCertification.issueDate === "object" &&
              updatedCertification.issueDate !== null &&
              typeof (updatedCertification.issueDate as any).getTime ===
                "function"
            ) {
              serializedCertification.issueDate = new Date(
                (updatedCertification.issueDate as any).getTime()
              ).toISOString()
            }
          }
          ...
          // Handle expirationDate if it exists - using a safer type checking approach
          if (
            "expirationDate" in updatedCertification &&
            updatedCertification.expirationDate
          ) {
            if (
              typeof updatedCertification.expirationDate === "object" &&
              updatedCertification.expirationDate !== null &&
              typeof (updatedCertification.expirationDate as any).getTime ===
                "function"
            ) {
              serializedCertification.expirationDate = new Date(
                (updatedCertification.expirationDate as any).getTime()
              ).toISOString()
            }
          }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `⭐️ PUT /api/career-data/career-summary - Request received for user ${userId}`
        )
        console.log(
          `📝 Career summary length: ${
        ...
        try {
          ...
          if (!updatedUser) {
            console.error(
              `❌ User ${userId} not found or update failed - storage.updateUser returned null/undefined`
            )
            return res.status(404).json({ message: "User not found" })
          }

          console.log(
            `✅ Career summary updated successfully for user ${userId}`
          )
          console.log(
            `📄 Updated user careerSummary length: ${
              updatedUser.careerSummary?.length || 0
            }`
          )
          ...
          console.error(
            "❌ Error stack:",
            updateError instanceof Error ? updateError.stack : "No stack trace"
          )
          res.status(500).json({
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedUser = await storage.updateUserLinkedInUrl(
          userId,
          linkedInUrl
        )
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Optimizing career data for user ${userId} based on job description`
        )
        ...
        const optimizedData = await optimizeCareerData(
          careerData,
          jobDescription
        )
