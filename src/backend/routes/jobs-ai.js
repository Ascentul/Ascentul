...
export const registerJobsAIRoutes = (router) => {
    // Route for AI-assisted job application suggestions
    router.post("/api/jobs/ai-assist", requireAuth, async (req, res) => {
        try {
            ...
            const suggestions = await generateJobApplicationSuggestions(jobTitle, companyName, jobDescription, resume?.content || "");
...
async function generateJobApplicationSuggestions(jobTitle, companyName, jobDescription, resumeContent) {
    try {
        // Prepare the prompt
        const prompt = constructJobAssistantPrompt(jobTitle, companyName, jobDescription, resumeContent);
        // Call OpenAI API
...
function constructJobAssistantPrompt(jobTitle, companyName, jobDescription, resumeContent) {
    return `
