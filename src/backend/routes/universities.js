...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        const universitiesWithCounts = (universityList || []).map((university) => {
            const studentCount = universityUsers?.filter((user) => user.university_id === university.id &&
                user.user_type === "university_student").length || 0;
            const adminCount = universityUsers?.filter((user) => user.university_id === university.id &&
                user.user_type === "university_admin").length || 0;
            return {
                ...university,
                studentCount,
                adminCount,
                // Map fields for frontend compatibility
                licensePlan: university.subscription_tier === "basic"
                    ? "Starter"
                ...
                licenseEnd: null,
                status: university.subscription_status === "active" ? "Active" : "Inactive",
...
router.patch("/:id", async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...updatedUniversity,
            // Map fields for frontend compatibility
            licensePlan: updatedUniversity.subscription_tier === "basic"
                ? "Starter"
            ...
            licenseEnd: null,
            status: updatedUniversity.subscription_status === "active"
                ? "Active"
                : "Inactive",
