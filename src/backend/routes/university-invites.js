...
router.get('/my-invites', async (req, res) => {
    ...
    try {
        ...
        const userInvites = await db.select({
            ...
            .from(invites)
            .where(and(eq(invites.email, user.email), eq(invites.status, "pending")));
        ...
        const invitesWithDetails = await Promise.all(userInvites.map(async (invite) => {
            ...
            if (invite.universityId) {
                const universities = await db.execute(sql `SELECT id, name, slug FROM universities WHERE id = ${invite.universityId}`);
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const universities = await db.execute(sql `SELECT * FROM universities WHERE id = ${validatedData.universityId}`);
        if (universities.length === 0) {
        ...
        const existingInvites = await db.execute(sql `SELECT * FROM invites 
        ...
        try {
            ...
            await sendUniversityInviteEmail(validatedData.email, token, university.name);
...
router.post('/accept/:inviteId', async (req, res) => {
    ...
    try {
        ...
        const invites = await db.execute(sql `SELECT * FROM invites 
          WHERE id = ${inviteId} 
          AND token = ${token}
          AND status = 'pending'`);
        if (invites.length === 0) {
        ...
        const universities = await db.execute(sql `SELECT * FROM universities WHERE id = ${invite.university_id}`);
        if (universities.length === 0) {
            return res.status(404).json({ error: 'University not found' });
        }
        const university = universities[0];
        ...
        await db.execute(sql `UPDATE invites 
          SET status = 'accepted', 
        ...
        if (invite.role === 'student' && university.license_used !== null) {
            await db.execute(sql `UPDATE universities 
