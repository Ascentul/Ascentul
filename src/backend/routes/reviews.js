...
import { db } from "../db";
import { z } from "zod";
import { requireAdmin, publicAccess } from "../utils/validateRequest";
const router = Router();
...
router.get("/", requireAdmin, async (req, res) => {
    try {
        ...
        if (search) {
            query = query.where(sql `${userReviews.feedback} ILIKE ${`%${search}%`} OR ${users.email} ILIKE ${`%${search}%`} OR ${users.name} ILIKE ${`%${search}%`}`);
        }
