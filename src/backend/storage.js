...
export class MemStorage {
    ...
    userIdCounter;
    goalIdCounter;
    workHistoryIdCounter;
    educationHistoryIdCounter;
    resumeIdCounter;
    coverLetterIdCounter;
    interviewQuestionIdCounter;
    interviewPracticeIdCounter;
    achievementIdCounter;
    userAchievementIdCounter;
    aiCoachConversationIdCounter;
    aiCoachMessageIdCounter;
    xpHistoryIdCounter;
    interviewProcessIdCounter;
    interviewStageIdCounter;
    followupActionIdCounter;
    contactMessageIdCounter;
    mentorChatConversationIdCounter;
    mentorChatMessageIdCounter;
    recommendationIdCounter;
    certificationIdCounter;
    userPersonalAchievementIdCounter;
    careerPathIdCounter;
    skillStackerPlanIdCounter;
    jobListingIdCounter;
    jobApplicationIdCounter;
    applicationWizardStepIdCounter;
    supportTicketIdCounter;
    projectIdCounter;
    networkingContactIdCounter;
    contactInteractionIdCounter;
    userReviewIdCounter;
    dailyRecommendationIdCounter;
    constructor() {
    ...
    // Cache operations
    async setCachedData(key, data, expirationMs) {
    ...
    async getAllActiveUsers() {
        ...
        return Array.from(this.users.values()).filter((user) => !user.subscriptionStatus ||
    ...
    async addUserXP(userId, amount, source, description) {
        const user = await this.getUser(userId);
        if (!user)
            throw new Error("User not found");
        // Only process XP for university users
        const isUniversityUser = user.userType === "university_student" ||
        ...
        if (calculatedLevel > newLevel) {
            ...
            if (newLevel >= 15)
                newRank = "Career Master";
            else if (newLevel >= 10)
                newRank = "Career Navigator";
            else if (newLevel >= 5)
                newRank = "Career Adventurer";
            else
                newRank = "Career Explorer";
        }
    ...
    async getEducationHistoryItem(id) {
    ...
    async updateCoverLetter(id, letterData) {
    ...
    async saveInterviewPractice(userId, practice) {
    ...
    async getUserReviews(userId) {
        return Array.from(this.userReviews.values()).filter((review) => review.userId === userId);
    }
    async createUserReview(userId, review) {
    ...
    async getNetworkingContacts(userId, filters) {
        ...
        if (filters?.query) {
            ...
            contacts = contacts.filter((contact) => contact.fullName.toLowerCase().includes(query) ||
    ...
    async getNetworkingContact(id) {
        return this.networkingContacts.get(id);
    }
    async createNetworkingContact(userId, contact) {
        const now = new Date();
        const newContact = {
    ...
    async updateNetworkingContact(id, contactData) {
    ...
    async getContactsNeedingFollowUp(userId) {
        ...
        return Array.from(this.networkingContacts.values())
            ...
            .sort((a, b) => {
            if (!a.lastContactedDate && !b.lastContactedDate)
                return 0;
            if (!a.lastContactedDate)
                return -1;
            if (!b.lastContactedDate)
                return 1;
    ...
    async createContactInteraction(userId, contactId, interaction) {
    ...
    async getUserPersonalAchievements(userId) {
        return Array.from(this.userPersonalAchievements.values())
            ...
            .sort((a, b) => new Date(b.achievementDate).getTime() -
    ...
    async updateUserPersonalAchievement(id, achievementData) {
    ...
    async deleteUserPersonalAchievement(id) {
        return this.userPersonalAchievements.delete(id);
    }
    async getUserAchievements(userId) {
        const userAchievementRecords = Array.from(this.userAchievements.values()).filter((ua) => ua.userId === userId);
    ...
    async getContactMessages() {
        return Array.from(this.contactMessages.values()).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    }
    ...
    async markContactMessageAsArchived(id) {
        const message = this.contactMessages.get(id);
        if (!message)
            return undefined;
    ...
    async updateInterviewProcess(id, processData) {
    ...
    async deleteInterviewProcess(id) {
    ...
    async getInterviewStages(processId) {
    ...
    async createInterviewStage(processId, stageData) {
    ...
    async updateInterviewStage(id, stageData) {
    ...
    async deleteInterviewStage(id) {
        ...
        const process = await this.getInterviewProcess(stage.processId);
        const followupActions = Array.from(this.followupActions.values()).filter((action) => action.stageId === id);
        for (const action of followupActions) {
            await this.deleteFollowupAction(action.id);
        }
    ...
    async getFollowupActions(processId, stageId) {
        let actions = Array.from(this.followupActions.values()).filter((action) => action.processId === processId);
    ...
    async getFollowupActionsByUser(userId) {
        ...
        const actions = Array.from(this.followupActions.values()).filter((action) => userProcesses.includes(action.processId));
        return actions.sort((a, b) => {
            // Sort by due date if available, otherwise by creation date
            const aDate = a.dueDate || a.createdAt;
            const bDate = b.dueDate || b.createdAt;
    ...
    async getFollowupAction(id) {
        return this.followupActions.get(id);
    }
    async createFollowupAction(processId, actionData) {
    ...
    async updateFollowupAction(id, actionData) {
    ...
    async completeFollowupAction(id) {
        const action = this.followupActions.get(id);
        if (!action)
            return undefined;
        if (action.completed) {
    ...
    async uncompleteFollowupAction(id) {
        const action = this.followupActions.get(id);
        if (!action)
            return undefined;
        if (!action.completed) {
    ...
    async getUserByPendingEmailToken(token) {
        return Array.from(this.users.values()).find((user) => user.pendingEmailToken === token);
    }
    ...
    async updateUserVerificationInfo(userId, verificationInfo) {
        const user = await this.getUser(userId);
        if (!user)
            return undefined;
        ...
        const updatedUser = {
            ...user,
            emailVerified: verificationInfo.emailVerified !== undefined
                ? verificationInfo.emailVerified
                : user.emailVerified,
            verificationToken: verificationInfo.verificationToken === null
                ? undefined
                : verificationInfo.verificationToken || user.verificationToken,
            verificationExpires: verificationInfo.verificationExpires === null
                ? undefined
    ...
    async updateUserLinkedInUrl(userId, linkedInUrl) {
        const user = await this.getUser(userId);
        if (!user)
            return undefined;
        const updatedUser = {
    ...
    async getMentorChatConversations(userId) {
        return Array.from(this.mentorChatConversations.values())
            .filter((conversation) => conversation.userId === userId)
            .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
    }
    async getMentorChatConversation(id) {
        return this.mentorChatConversations.get(id);
    }
    async createMentorChatConversation(userId, conversation) {
    ...
    async getMentorChatMessages(conversationId) {
        return Array.from(this.mentorChatMessages.values())
            .filter((message) => message.conversationId === conversationId)
            .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    }
    async addMentorChatMessage(message) {
    ...
    async generateDailyRecommendations(userId) {
        ...
        try {
            ...
            if (newRecommendations.length < 5) {
                ...
                while (newRecommendations.length < 5 &&
    ...
    async clearTodaysRecommendations(userId) {
    ...
    async saveCareerPath(userId, name, pathData) {
    ...
    async getSkillStackerPlan(id) {
        return this.skillStackerPlans.get(id);
    }
    async getSkillStackerPlanByGoalAndWeek(goalId, week) {
        return Array.from(this.skillStackerPlans.values()).find((plan) => plan.goalId === goalId && plan.week === week);
    }
    async getAllSkillStackerPlans(userId) {
        return Array.from(this.skillStackerPlans.values()).filter((plan) => plan.userId === userId);
    }
    async getSkillStackerPlansByGoal(goalId) {
        return Array.from(this.skillStackerPlans.values()).filter((plan) => plan.goalId === goalId);
    }
    async createSkillStackerPlan(userId, plan) {
    ...
    async updateSkillStackerPlan(id, planData) {
    ...
    async updateSkillStackerTaskStatus(planId, taskId, status, rating) {
    ...
    async completeSkillStackerWeek(planId) {
        const plan = this.skillStackerPlans.get(planId);
    ...
    async deleteSkillStackerPlan(id) {
    ...
    async getJobListings(filters) {
        ...
        if (filters) {
            if (filters.query) {
                ...
                listings = listings.filter((listing) => listing.title.toLowerCase().includes(lowercaseQuery) ||
            ...
            if (filters.location) {
                ...
                listings = listings.filter((listing) => listing.location.toLowerCase().includes(lowercaseLocation));
            }
    ...
    async createJobListing(listing) {
    ...
    async updateJobListing(id, listingData) {
    ...
    async getJobApplications(userId) {
        return Array.from(this.jobApplications.values()).filter((application) => application.userId === userId);
    }
    async getJobApplication(id) {
        return this.jobApplications.get(id);
    }
    async createJobApplication(userId, application) {
    ...
    async submitJobApplication(id, applied = true) {
        const application = this.jobApplications.get(id);
        if (!application)
            return undefined;
    ...
    async getInterviewStagesForApplication(applicationId) {
        return Array.from(this.interviewStages.values())
            .filter((stage) => {
    ...
    async getFollowupActionsForApplication(applicationId) {
    ...
    async createFollowupActionForApplication(applicationId, actionData) {
        const id = this.followupActionIdCounter++;
        const now = new Date();
        const newAction = {
            id,
    ...
    async getApplicationWizardSteps(applicationId) {
    ...
    async getApplicationWizardStep(id) {
        return this.applicationWizardSteps.get(id);
    }
    async createApplicationWizardStep(applicationId, step) {
    ...
    async updateApplicationWizardStep(id, stepData) {
    ...
    async completeApplicationWizardStep(id) {
        const step = this.applicationWizardSteps.get(id);
        if (!step)
            return undefined;
        const completedStep = {
    ...
    async getNetworkingContacts(userId, filters) {
        ...
        if (filters) {
            if (filters.query) {
                const query = filters.query.toLowerCase();
                contacts = contacts.filter((contact) => contact.name.toLowerCase().includes(query) ||
                    (contact.company &&
                        contact.company.toLowerCase().includes(query)) ||
                    (contact.position &&
    ...
    async getContactsNeedingFollowUp(userId) {
        ...
        const contactsWithBuiltInFollowup = Array.from(this.networkingContacts.values()).filter((contact) => contact.userId === userId &&
    ...
    async updateContactInteraction(id, data) {
    ...
    async deleteContactInteraction(id) {
...
export class DatabaseStorage {
    ...
    async submitJobApplication(id, applied = false) {
        try {
    ...
    async updateCertification(id, certificationData) {
        try {
            ...
            const updateData = { ...certificationData };
            const now = new Date();
            ...
            if (updateData.expirationDate) {
                ...
                else if (typeof updateData.expirationDate !== "string" &&
    ...
    async getNetworkingContacts(userId, filters) {
        ...
        // Apply filters if provided
        if (filters?.query) {
            query = query.where(sql `${networkingContacts.fullName} ILIKE ${"%" + filters.query + "%"} OR 
            ${networkingContacts.company} ILIKE ${"%" + filters.query + "%"} OR 
            ${networkingContacts.email} ILIKE ${"%" + filters.query + "%"}`);
        }
        if (filters?.relationshipType) {
            query = query.where(eq(networkingContacts.relationshipType, filters.relationshipType));
        }
    ...
    async completeContactFollowUp(id) {
        try {
            ...
            if (!followUp)
                return undefined;
            if (followUp.completed)
                return followUp; // Already completed
    ...
    async completeRecommendation(id) {
      ...
      `, [id]);
            ...
            const [updatedRecommendation] = await db
                ...
                .returning();
            // Award XP for completing a recommendation
            await this.addUserXP(recommendation.userId, 15, // 15 XP for completing a recommendation
            "daily_recommendation", `Completed daily recommendation: ${recommendation.text}`);
            return updatedRecommendation;
        }
        catch (error) {
...
export async function checkStorageHealth() {
    if (storage instanceof SupabaseStorage) {
        try {
            ...
            const connectionOk = await storage.testDatabaseConnection();
            return {
                type: "supabase",
            ...
            return {
                type: "supabase",
                status: "failing",
                details: `Supabase storage error: ${error.message}`
            };
        }
    }
