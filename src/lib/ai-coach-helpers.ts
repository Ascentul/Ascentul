...
function sanitizeForPrompt(input: string, maxLength = 500): string {
  ...
  return stripped.replace(/---/g, '—').replace(/\s+/g, ' ').trim().slice(0, maxLength);
}
...
export function buildUserContext(data: UserContextData): string {
  ...
  if (data.userProfile) {
    ...
    if (data.userProfile.name)
      contextParts.push(`Name: ${sanitizeForPrompt(data.userProfile.name, 100)}`);
    if (data.userProfile.current_position)
      contextParts.push(
        `Current Position: ${sanitizeForPrompt(data.userProfile.current_position, 150)}`,
      );
    if (data.userProfile.current_company)
      contextParts.push(
        `Current Company: ${sanitizeForPrompt(data.userProfile.current_company, 150)}`,
      );
    if (data.userProfile.industry)
      contextParts.push(`Industry: ${sanitizeForPrompt(data.userProfile.industry, 120)}`);
    if (data.userProfile.experience_level)
      contextParts.push(
        `Experience Level: ${sanitizeForPrompt(data.userProfile.experience_level, 120)}`,
      );
    if (data.userProfile.skills)
      contextParts.push(`Skills: ${sanitizeForPrompt(data.userProfile.skills)}`);
    if (data.userProfile.career_goals)
      contextParts.push(`Career Goals: ${sanitizeForPrompt(data.userProfile.career_goals)}`);
  }
  ...
  if (data.goals && data.goals.length > 0) {
    ...
    data.goals.slice(0, MAX_GOALS_IN_CONTEXT).forEach((goal, idx) => {
      contextParts.push(
        `${idx + 1}. ${sanitizeForPrompt(goal.title, 200)} (Status: ${sanitizeForPrompt(goal.status ?? 'unknown', 80)})`,
      );
    });
  }
