 ...
 */
export function logVoiceEvent(event: string, message: string, data?: any) {
 ...
 */
export function formatBase64ForLogging(base64String: string) {
  if (!base64String) return 'Empty string';
 ...
 */
export async function testAudioCapture(durationMs: number = 1000): Promise<{
  success: boolean;
  error?: string;
  recordingStats?: {
    durationMs: number;
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        recorder.addEventListener('stop', async () => {
          ...
          let base64Length: number | undefined = undefined;
          try {
            const reader = new FileReader();
            const base64Promise = new Promise<string>((resolveBase64) => {
              reader.onloadend = () => {
                resolveBase64(reader.result as string);
              };
            });
 ...
 */
export function saveDebugData(key: string, data: any) {
  try {
