 ...
 */
export function loadInterviewStagesForApplication(applicationId) {
    if (!applicationId)
        return [];
    ...
    const interviewStages = interviewStagesJson ? JSON.parse(interviewStagesJson) : [];
 ...
 */
export function saveInterviewStagesForApplication(applicationId, stages) {
    if (!applicationId || !stages)
        return;
    const stagesKey = `${MOCK_STAGES_PREFIX}${applicationId}`;
    const interviewStagesKey = `${MOCK_INTERVIEW_STAGES_PREFIX}${applicationId}`;
 ...
 */
export function addInterviewStageForApplication(applicationId, stage) {
    if (!applicationId || !stage)
        throw new Error('Application ID and stage are required');
 ...
 */
export function updateInterviewStage(stage) {
 ...
 */
export function loadAllInterviewStages() {
    const allStages = {};
 ...
 */
function updateApplicationInterviewStatus(applicationId) {
    const applications = JSON.parse(localStorage.getItem('mockJobApplications') || '[]');
    const appIndex = applications.findIndex((app) => app.id.toString() === applicationId.toString());
    if (appIndex !== -1) {
 ...
 */
export function getInterviewingApplications() {
    const applications = JSON.parse(localStorage.getItem('mockJobApplications') || '[]');
 ...
 */
export function notifyInterviewDataChanged() {
