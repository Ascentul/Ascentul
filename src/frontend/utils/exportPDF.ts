...
export function exportCoverLetterToPDF(): void {
  try {
    ...
    const headerContainer = previewLetterEl.querySelector(
      ".text-base.font-normal.text-neutral-900"
    )
    ...
    if (!headerContainer) {
      ...
      const fullNameElement = previewLetterEl.querySelector(
        "h2, p.font-semibold"
      )
      fullName = fullNameElement?.textContent?.trim() || "Your Name"

      const jobTitleElement = previewLetterEl.querySelector(
        ".text-base.font-normal.mt-1"
      )
      jobTitle = jobTitleElement?.textContent?.trim() || ""

      const contactInfoElement = previewLetterEl.querySelector(
        "p.text-neutral-700, div.text-base.font-normal.mt-4"
      )
      contactInfo = contactInfoElement?.textContent?.trim() || ""

      // Date from any element with date format
      const dateContainer = Array.from(
        previewLetterEl.querySelectorAll("div, p")
      ).find((el) => {
      ...
      date =
        dateContainer?.textContent?.trim() || new Date().toLocaleDateString()

      // Get company name - check multiple possible selectors
      const companyElement =
        previewLetterEl.querySelector(".mt-1.mb-6.text-base.font-normal p") ||
      ...
      date =
        paragraphs[3]?.textContent?.trim() || new Date().toLocaleDateString()
    ...
    // Get body content
    const letterBodyElement = previewLetterEl.querySelector(
      ".whitespace-pre-wrap.text-base.font-normal"
    )
    ...
    letterBody = letterBody
      ...
      .replace(
        /\s*(Sincerely|Best regards|Regards|Yours truly|Thank you|Best|Respectfully)[,\s]+(.*?)$/i,
        ""
      )
    ...
    if (letterBody.length > 200) {
      ...
      const lastFewHundredChars = letterBody
        .substring(letterBody.length - 200)
        .toLowerCase()
    ...
    if (contactInfo) {
      ...
      if (window.linkedInProfile) {
        ...
        if (emailMatch) {
          ...
          const allEmails = Array.from(previewLetterEl.querySelectorAll("p"))
            .map((p) => {
        ...
        console.log(
          "Using standard contact info (no LinkedIn URL):",
          contactInfo
        )
      }
    }
    ...
    const greetingParagraph = Array.from(
      previewLetterEl.querySelectorAll("p")
    ).find((el) => {
      const text = el.textContent?.trim() || ""
...
export function exportResumeToPDF(
  ...
  try {
    ...
    const nameElement =
      resumeElement.querySelector("h1, h2") ||
      resumeElement.querySelector(".resume-name")
    ...
    const contactInfo =
      resumeElement
        .querySelector(".contact-info, .personal-info")
        ?.textContent?.trim() || ""

    // Extract sections (summary, experience, education, skills)
    const summary =
      resumeElement
        .querySelector(".summary-section, .professional-summary")
        ?.textContent?.trim() || ""
    ...
    if (skillsSection) {
      ...
      const skillTags = Array.from(
        skillsSection.querySelectorAll(".skill-tag, .skill")
      )
    ...
    resumeElement
      .querySelectorAll(".experience-item, .work-item")
      .forEach((item) => {
    ...
    resumeElement
      .querySelectorAll(".education-item, .edu-item")
      .forEach((item) => {
    ...
    const filename = `resume_${cleanName}_${
      new Date().toISOString().split("T")[0]
    }.pdf`
...
export function exportElementToPDF(
  ...
  try {
    ...
    if (window.linkedInProfile) {
      console.log(
        "Found LinkedIn profile URL for PDF export:",
      ...
      if (contactInfo) {
        ...
        const hasLinkedInText =
          contactInfo.textContent?.includes("LinkedIn") || false

        if (hasLinkedInText) {
          console.log(
            "LinkedIn text found in contact info, converting to link for PDF"
          )
          ...
          console.log(
            "Reformatted contact info for PDF with LinkedIn link:",
            newContent
          )
        } else {
        ...
        const headerSection = clone.querySelector(
          ".header, header, .letterhead, h1, h2"
        )
        if (headerSection) {
      ...
      if (bodyContent) {
        ...
        if (
          bodyHTML.includes("{{LINKEDIN_URL}}") ||
