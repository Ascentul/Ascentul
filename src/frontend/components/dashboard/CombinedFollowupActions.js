...
export function CombinedFollowupActions({ limit = 5, showTitle = true }) {
    ...
    const contactFollowupsRef = useRef([]);
    ...
    const loadApplicationFollowups = useCallback(async (apps = []) => {
                ...
                if (apiResponse.ok) {
                    ...
                    if (followups && Array.isArray(followups)) {
                        followups
                            ...
                            const exists = pendingFollowups.some((pf) => pf.source === "application" && pf.id === followup.id);
                            if (!exists) {
                                pendingFollowups.push({
                                    ...followup,
                                    applicationId: followup.applicationId || app.id,
    ...
    const processContactFollowups = useCallback((contactFups = []) => {
        const currentContactFollowups = contactFups;
        if (!Array.isArray(currentContactFollowups))
            return [];
    ...
    const sortFollowups = useCallback((followups) => {
    ...
    const refreshFollowups = useCallback(async (apps, contacts) => {
        try {
            setIsLoading(true);
                ...
                try {
                    ...
                    if (followupsJson) {
                        ...
                        if (Array.isArray(followups)) {
                            ...
                            const updatedLocalFollowups = followups.map((f) => f.id === followup.id
                            ...
                            setTimeout(() => refreshFollowups(applicationsRef.current, contactFollowupsRef.current), 300);
                            toast({
                                title: "Followup updated",
                                description: "The followup status has been updated successfully."
                            });
    ...
    const filteredFollowups = useMemo(() => {
        ...
        return followupActions.filter((followup) => followup.source ===
