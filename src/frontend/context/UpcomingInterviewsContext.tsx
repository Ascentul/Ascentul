...
export function UpcomingInterviewsProvider({
  ...
  const updateInterviewCount = useCallback(async (): Promise<number> => {
    try {
        ...
        try {
          ...
          stages.forEach((stage: any) => {
            ...
            if (!allStagesSet.has(stage.id)) {
              ...
              const statusInfo = `status=${stage.status || "unset"}, outcome=${
                stage.outcome || "unset"
              }`
      ...
      console.log(
        `Found ${scheduledStageIds.size} upcoming scheduled interviews`
      )

      if (validStages.length > 0) {
        console.log(
          "Valid upcoming interviews:",
      ...
      console.log(
        `Local count: ${appCount} interviewing applications, ${scheduledInterviewsCount} scheduled interviews`
      )
      ...
      try {
        ...
        if (Array.isArray(apiApplications)) {
          ...
          const apiInterviewingApps = apiApplications.filter(
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleInterviewUpdate as EventListener
    )
    window.addEventListener(
      APPLICATION_STATUS_CHANGE_EVENT,
      handleApplicationStatusChange as EventListener
    )
    window.addEventListener(
      INTERVIEW_STAGE_CHANGE_EVENT,
      handleInterviewStageChange as EventListener
    )
...
export function useUpcomingInterviews() {
  ...
  if (context === undefined) {
    throw new Error(
      "useUpcomingInterviews must be used within an UpcomingInterviewsProvider"
    )
  }
  return context
}
