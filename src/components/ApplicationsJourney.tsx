...
import { ArrowRight, Bookmark, Briefcase, MessageSquare, Plus, Send, Trophy } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col items-center relative group">
      ...
      <div className="mt-2 text-center">
        <p className={cn('text-lg font-semibold', hasItems ? 'text-slate-900' : 'text-slate-400')}>
          {count}
        </p>
          ...
          <div className="bg-slate-900 text-white text-xs rounded-lg px-3 py-2 shadow-lg whitespace-nowrap">
            {recentApps.slice(0, 2).map((app, idx) => (
              <div key={idx} className="py-0.5">
                {app.company} –{' '}
                {app.job_title.length > 20 ? app.job_title.slice(0, 20) + '...' : app.job_title}
              </div>
            ))}
            {recentApps.length > 2 && (
...
function getCoachingCopy(funnelData: Record<string, number>): {
  text: string;
  ctaText: string;
  ctaHref: string;
} {
    ...
    <section className="h-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col">
      <div className="flex items-center justify-between px-5 py-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Track your progress through the hiring funnel</p>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              <p className="text-sm text-slate-600 mb-3">{coaching.text}</p>
