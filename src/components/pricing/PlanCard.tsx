...
const ctaButtonClasses =
  'mt-8 w-full h-11 rounded-xl bg-black text-white hover:bg-black/90 hover:shadow-md hover:-translate-y-[1px] transition-all duration-200';
...
export function PlanCard({
  ...
  return (
    ...
    >
      {highlighted && (
        <span
          className="absolute -top-3 left-6 rounded-full bg-brand-blue text-white px-2 py-0.5 text-[11px] font-medium shadow-sm"
      ...
      <div className="mt-4 flex items-baseline gap-1">
        <span className="text-5xl font-extrabold tracking-tight text-black">{price}</span>
      ...
      {subline && (
        <p className="mt-1 text-xs text-neutral-500">
          {subline}
          {savings && <span className="ml-2 text-brand-blue font-medium">{savings}</span>}
        </p>
      )}

      {hasTrial && (
        <div
          className="mt-2 inline-flex items-center gap-1 rounded-full border border-brand-blue/25 bg-brand-blue/8 px-2 py-0.5 text-xs font-medium text-brand-blue"
          aria-label="7 day free trial included"
        >
          <BadgeCheck className="h-3 w-3" aria-hidden="true" />7 day free trial
        </div>
      )}
