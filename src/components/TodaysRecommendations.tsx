...
export function TodaysRecommendations() {
    ...
    if (recommendationsHash !== prevRecommendationsHash.current) {
      ...
      setLocalRecommendations((prev) => {
        ...
        return recommendationsArray.map((rec) => {
          ...
          const completedAt =
            prevRec?.completedAt ?? (completed ? (rec.completedAt ?? null) : null);
  ...
  const sortedRecommendations = useMemo(() => {
    return [...localRecommendations].sort((a, b) => {
      return a.completed === b.completed ? a.type.localeCompare(b.type) : a.completed ? 1 : -1;
    });
    ...
    <section className="h-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col">
      <div className="flex items-center justify-between px-5 py-3">
        <div>
          ...
          <p className="text-xs text-slate-500">AI-powered suggestions for your career</p>
        </div>
        <Button
          variant="outline"
          size="sm"
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="flex items-start rounded-xl border border-slate-100 bg-white p-3"
              >
