...
import { Clock, FileEdit, FileText, Filter, Search, User } from 'lucide-react';
...
export function ReviewQueue({ reviews, isLoading }: ReviewQueueProps) {
  ...
  const filteredReviews = useMemo(() => {
    return reviews.filter((review) => {
      ...
      const matchesType = typeFilter === 'all' || review.asset_type === typeFilter;
      ...
      {filteredReviews.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredReviews.map((review) => {
            ...
            const statusColor =
              statusColors[review.status as keyof typeof statusColors] || statusColors.default;
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div>
                          <h3 className="font-medium text-sm">{review.asset_name}</h3>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/advisor/students/${review.student_id}`}>View Student</Link>
                        </Button>
                      </div>
                    </div>
                  </div>
